...
188 core tests pass, and the whole loop has been driven end to end: a `unifieddev://` deep link
...
Three live tests (`UD_LIVE=1 ./Tools/test-core.sh LiveAgent`) run against the real `claude`
...
- A `unifieddev://` link opened a second window, because a `WindowGroup` makes one per URL.
...
- Core logic (git engine, NDJSON decoding, diff parsing) lives in a `Core` target with
  tests. SwiftUI views live in `Unified Dev` and are not unit tested.
...
**Endgoal:** `./Tools/build.sh && open .build/UnifiedDev.app` shows a window with sidebar, center and
inspector.
...
`UD_*` vars. Setup and Run script tabs.
