...
Unified Dev separates its tested behaviour from its window. Read `CLAUDE.md` for the enforced rules and
`RELEASING.md` for distribution. Build an isolated dev copy; never test against the installed
production app or its database.

## Boundaries

- `Core` owns persistence, processes, agent protocols, workspace operations and pure
  presentation decisions. It cannot import a UI framework.
- `Unified Dev` owns SwiftUI and AppKit integration. Main-actor observable models coordinate the window;
  views render state and forward intent rather than running shell commands.
- `bridge` relays MCP requests to the running app. The registry establishes caller authority;
...
These checks constrain Unified Dev's own operations. They are not an operating-system sandbox against
