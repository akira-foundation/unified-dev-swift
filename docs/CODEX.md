...
**Unified Dev drives `codex app-server --listen stdio://`, which is JSON-RPC 2.0 over stdio. It never
...
- **No text deltas.** Events are item-granular, so a reply appears all at once when it is finished.
  Unified Dev's transcript types as the model does. `exec` cannot do that.
...
- **No approvals at all.** Run with `-s read-only -c approval_policy=on-request` and a prompt that
  ...
  no wire on which a question could arrive. That is the same failure Unified Dev already fixed once for
...
`Sources/Core/Agent/Codex/CodexProtocol.swift`, because `exec` will keep looking tempting.
...
developer_instructions } }`. Unified Dev now declares `experimentalApi: true` at initialise so the
...
The same installed version supports two conversation-history contracts. Unified Dev reads
...
Unified Dev owns the independent file/index snapshots and recovery journal.
...
| Attachments | `UserInput` has a `localImage` variant taking a path. Unified Dev's existing approach works unchanged |
...
sentence saying the file should be named differently, it wrote the different name. So Unified Dev sends
...
**The efforts belong to the model.** Unified Dev's flat five-entry list in `ComposerOption.efforts` is
...
**Nothing in Unified Dev changed to offer the new generation.** It was fetched, drawn under the
...
alone, with `"default"` meaning standard speed. Unified Dev reads `config/read` for the composer's
project and combines `service_tier` with the selected model's advertised tiers. A missing Unified Dev
...
stored explicitly, since omitting the field would leave a configured fast tier enabled. Unified Dev
...
Four new files in `Sources/Core/`, nothing else touched:
...
- **`CodexEvent.swift`**, the event model. `CodexItem` over the eighteen thread item types, ten of
  ...
  `CodexTokenUsage` (with `agentUsage`, mapping onto Unified Dev's own `AgentUsage`), `CodexApprovalRequest`
...
Tests: `Tests/CoreTests/CodexProtocolTests.swift` and `CodexClientTests.swift`, 42 of them,
...
`turn/interrupt` ends a turn. **Nothing on this protocol ends the process**, so Unified Dev has to, and
...
was missing: at the moment Unified Dev's quit path reported the agent gone, `codex` (a node script) and
...
`Sources/Core/Agent/ToolPresenter.swift` is 563 lines of Claude Code tool names
...
and Unified Dev's `DiffParser` reads that shape.
...
| Unified Dev mode | Codex preset | `approvalPolicy` | `sandbox` | `approvalsReviewer` |
| --- | --- | --- | --- | --- |
...
**Unified Dev shipped without that row, and a user said so**, having read the Codex app's picker beside
Unified Dev's: three rows in Claude Code's vocabulary, and the one he wanted, the one the Codex app calls
...
`auto_review`, `guardian_subagent` ``. The third is the older spelling of the second and Unified Dev does
not offer it.
...
**What Unified Dev does not yet draw.** With `auto_review` the server also emits
`item/autoApprovalReview/started` and `.../completed`, and offers
`thread/approveGuardianDeniedAction` so a person can overrule a denial the subagent made. Unified Dev
...
picker's footnote as a mode Codex does not have, on the argument that somebody who knows Unified Dev has
...
in the core. The same user's report is the reason: Unified Dev was printing "Ask, Accept edits, Full
...
the mode Codex calls Approve for me; Unified Dev had been labelling it "Ask". It reads "Auto" now, which
...
live in Unified Dev's own `permission_grants` keyed by repository and matched on **exact equality**, a
matching grant is answered by Unified Dev itself with a note in the transcript saying so, and **no
...
- **`acceptForSession` is Codex's "do not ask again", and the server remembers it, not Unified Dev.**
...
- **Codex offers no rule of its own.** Claude Code's CLI sends `permission_suggestions`, its own
  judgement about which rule would let calls like this through. Codex sends nothing, so Unified Dev
  ...
  Unified Dev granting something nobody agreed to.
...
| Unified Dev | On the wire |
| --- | --- |
| Allow once | `accept` |
| Allow for this session | `acceptForSession` |
| Always allow (project) | `acceptForSession`, plus a row in Unified Dev's own `permission_grants` |
...
flag and Unified Dev infers it; here it is handed over.
...
- **The effort picker follows the model, within the chat.** Codex efforts differ per model, so
  ...
  Unified Dev's `high`. `CodexModel.resolvedEffort(preferring:)` already does the fallback.
