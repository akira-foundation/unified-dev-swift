# Working with code in Unified Dev
...
`Sources/App.swift:42` and Markdown file links open inside Unified Dev. Back and Forward remember file
...
Usages are both available in the contextual menu. Unified Dev looks for
...
For PHP and Blade files inside a Laravel app, Unified Dev also queries
...
Unified Dev forwards file changes to refresh Laravel's index and disables Pest helper generation.
...
For development, `Unified Dev --source-editor-probe` checks the native editor in an unshown window and
writes light and dark screenshots under `/tmp/editor-*.png`. Add `--language-server` to check a
real SourceKit-LSP lookup between two temporary Swift files. Use an isolated `UD_DB_PATH`.

`./Tools/test-core.sh EditorExperienceTests WorktreeWatcherTests` covers navigation and file
watching. Set `UD_LOCAL_LSP=1` for the real Intelephense check. To also test Laravel view links,
set `UD_LARAVEL_LSP_VENDOR` to the `vendor` directory of a disposable Laravel app with Tinker
