...
- Use `NavigationStack`, `NavigationSplitView` and the modern `Tab` API where those containers
  match the screen. Unified Dev also has intentional custom workspace and window navigation.
...
- Generated asset symbols require build support. Unified Dev builds through SwiftPM and custom scripts;
...
- SwiftUI's WebKit views can replace some wrappers on macOS 26. Compare delegate, navigation and
  embedding requirements before changing Unified Dev's existing web view bridge.
