...
description: Release Unified Dev through GitHub and publish generated release notes on unified-dev.akira-io.com, or create signed local release artefacts. Use when asked to cut, ship, publish, or prepare a Unified Dev release.
---
...
3. Prepare the tag and notes before publishing. A request to prepare a release stops at a
   ...
   ```sh
   gh release create <tag> --repo akira-io/unifieddev --verify-tag --title 'Unified Dev <version>' --notes-file <notes-file>
   ```
...
5. **Publish the generated release notes on unified-dev.akira-io.com as part of the release.** Follow
   [the website procedure](../../../RELEASING.md#publish-the-website-release-notes). The website
   lives in `akira-io/unified-dev.akira-io.com`; the app workflow does not publish its changelog. Importing a
...
6. Verify the GitHub release and both assets, their public bucket URLs, the appcast version/build
   and channel, and `https://unified-dev.akira-io.com/changelog#<tag>`. For a stable release, also verify the
...
does not imply publishing. For an isolated app to test, use `unifieddev-dev-build` instead.
