#!/usr/bin/env python3
"""Builds ~/Desktop/unifieddev-icon-02-piece.html: 02-piece at several amounts of
...
def piece(up, down, x1):
    ...
    def fn(small=False):
        ...
        lanes = shine.split_lanes(
            sp,
            afill=shine.fill("accent", u, dn, *w["a"]),
        ...
        body = depth.base_flat(
            sp,
            lanes=shine.cut(sp, sp["a"](shine.paint("accent", u, dn, *w["a"]))
...
def page(rows):
    out = os.path.expanduser("~/Desktop/unifieddev-icon-02-piece.html")
    h = [HEAD]
    for r in rows:
        u, d = r["meas"]
        uf, df = r["flatmeas"]
...
<title>Unified Dev 02-piece, the leaving edge</title><style>
