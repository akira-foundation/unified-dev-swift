...
D = os.path.expanduser("~/Desktop/unifieddev-icons-v7")
...
def rim(small=False):
    ...
    return wrap("".join([
        ground(flat("deep")),
        thick(disc, sheen("unifieddev"), flat("current")),
        contact(bite, disc, blur=13, alpha=0.34),
        thick(bite, sheen("accent"), flat("fathom")),
    ]))
...
def deck(small=False):
    ...
    prev = None
    for i, col in enumerate(("accent", "current", "shallow")):
    ...
    o.append(thick(bite, sheen("accent"), flat("fathom")))
    return wrap("".join(o))
...
def crop(small=False):
    ...
    return wrap("".join([
        ground(sheen("unifieddev", 0.06, 0.12)),
        contact(bite, sh_tile(), d=(30, 38), blur=16, alpha=0.30),
        thick(bite, sheen("accent"), flat("fathom"), d=(20, 30)),
    ]))
...
def lift(small=False):
    ...
    return wrap("".join([
        ground(flat("abyss")),
        thick(disc, sheen("shallow"), flat("unifieddev")),
...
def chip(small=False):
    ...
    o.append(puck(420, 396, r1, r1, 72, sheen("unifieddev"), flat("current")))
    ...
    o.append(puck(792, 566, r2, r2, 60, sheen("accent"), flat("fathom")))
    return wrap("".join(o))
...
def cant(small=False):
    ...
    o.append(puck(852, 664, 356, 238, 70, sheen("accent"), flat("fathom")))
    return wrap("".join(o))
...
def cleave(small=False):
    ...
    fig = thick(disc, sheen("shallow"), flat("unifieddev")) + \
...
def coins(small=False):
    ...
    for i, col in enumerate(("accent", "current", "shallow")):
        ...
        o.append(thick(c, sheen(col), flat({"accent": "fathom", "current": "accent",
                                            "shallow": "unifieddev"}[col])))
        prev = c
    return wrap("".join(o))
...
def copies(small=False):
    ...
    for i, col in enumerate(("accent", "current", "shallow")):
        ...
        if i == 2:
            ...
            o.append(hole(sheen("accent", 0.03, 0.06)))
...
def lanes(small=False):
    ...
    is a beach towel."""
    o = [ground(flat("abyss"))]
    plan = ((188, 196, "shallow", "unifieddev"), (404, 124, "current", "accent"),
            (606, 238, "shallow", "unifieddev"), (858, 146, "current", "accent"))
...
def cross(small=False):
    ...
    return wrap("".join([
        ground(flat("deep")),
        thick(a, sheen("current"), flat("accent")),
        contact(b, a, d=(22, 28), blur=14, alpha=0.36),
        thick(b, sheen("shallow"), flat("unifieddev")),
    ]))
...
def well(small=False):
    ...
    plan = ((78, "accent"), (176, "fathom"), (282, "shallow"))
    outer = sh_tile()
...
def slip(small=False):
    ...
    plate = "".join([
        ...
        thick(sh_circle(452, 440, 356 if not small else 374),
              sheen("shallow"), flat("unifieddev")),
        sh_circle(818, 850, 380)(sheen("accent")),
    ])
