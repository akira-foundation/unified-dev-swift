...
piece on the white instead of a pale one.
  ...
  03 wick     the Accent lane crosses IN over the left margin, the bar is cut
...
def figure(m, small, bar="off", capx=None, panel=None, ground="foam",
    ...
    lane    the upper Accent lane crosses OUT over the upper left of the margin
    ...
    tile = sh_squircle()
    lanes = a0(sheen("accent")) + b0(sheen("current"))
    inpanel = (clipped(b0(sheen("current")), p) + a(sheen("accent"))
...
def wick(small=False):
    ...
    lies on the white here is Accent, which measures 4.76 against Foam and
...
def through(small=False):
    ...
    entering lane is Accent and reads on the white on its own; the leaving bar
...
def unframed(small=True):
    ...
    body = "".join([
        fullbleed(flat("deep")), a(sheen("accent")), b(sheen("current")),
