...
Writes two things into Resources/:
  ...
  UnifiedDev.icon/         the layered document macOS 26 draws, where the system
                      supplies the material, the shadow and the specular and
                      generates the dark and tinted variants itself
  AppMenuBar.pdf    the menu bar mark, drawn by menubar.py from the same lane
...
a floor of macOS 26 the system finds `UnifiedDev.icon` through CFBundleIconName and
...
def layered():
    ...
    its own gradient and one fill key cannot say Accent and Current at once.
    ...
    bundle = os.path.join(RESOURCES, "UnifiedDev.icon")
