...
Foam ground, same Deep panel with its spur, same Accent and Current lanes, same
...
its effect LIVES:
  ...
  system   the artwork is not painted at all. The effect is asked for through
           ...
           `UnifiedDev.icon` can do this, so the flat `.icns` gets the nearest thing
           paint can say
...
# cannot carry Accent and Current at once. Splitting a group is not a change to
...
def document(sp, mark=None, lanes=None, panel=None, ground=None,
    ...
    if lanes is None:
        lanes = [layer("Bleed", clipped(sp["a"](sheen("accent"))
                                        + sp["b"](sheen("current")),
                                        lanewindow(sp)))]
...
def split_lanes(sp, afill=None, bfill=None, aop=None, bop=None, blend=None):
    ...
    return [layer("Current", clipped(sp["b"](sheen("current")), p), bfill, bop, blend),
            layer("Accent", clipped(sp["a"](sheen("accent")), p), afill, aop, blend)]
...
def v00_current(small=False):
    ...
    sp = shapes(small)
    return document(sp), base_flat(sp, lanes=cut(sp, sp["a"](sheen("accent"))
                                               + sp["b"](sheen("current"))))
...
def v01_sheet(small=False):
    ...
    up, dn = 0.085, 0.085
    lanes = split_lanes(sp, afill=fill("accent", up, dn), bfill=fill("current", up, dn))
    groups = document(
        sp,
    ...
    body = base_flat(
        sp,
        lanes=cut(sp, sp["a"](g("accent")) + sp["b"](g("current"))),
...
def v02_piece(small=False):
    ...
    body = base_flat(
        sp,
        lanes=cut(sp, sp["a"](paint("accent", up, dn, *w["a"]))
...
def v03_crest(small=False):
    ...
    up = 0.30
    lanes = split_lanes(sp,
                        afill=fill("accent", up, 0, *crest(*w["a"])),
    ...
    body = base_flat(
        sp,
        lanes=cut(sp, sp["a"](paint("accent", up, 0, *crest(*w["a"])))
...
def v04_rim(small=False):
    ...
    lanes = split_lanes(sp,
                        afill=fill("accent", 0.26, 0, *crest(*w["a"], at=0.06)),
    ...
    return groups, base_flat(sp, lanes=cut(sp, sp["a"](sheen("accent"))
                                           + sp["b"](sheen("current"))),
                             after_bar=rims(1.0))
...
def v06_wash(small=False):
    ...
    lanes = cut(sp, '<g opacity="0.82">%s</g>'
                % (sp["a"](sheen("accent")) + sp["b"](sheen("current"))))
...
def v07_seat(small=False):
    ...
    lanes = split_lanes(sp,
                        afill=fill("accent", 0, 0.24, *base(*w["a"], at=0.26)),
...
def v09_liquid(small=False):
    ...
    body = base_flat(
        sp,
        lanes=cut(sp, sp["a"](sheen("accent")) + sp["b"](sheen("current"))),
...
def v10_shine(small=False):
    ...
    lanes = split_lanes(sp,
                        afill=fill("accent", 0.16, 0.12, *ramp(*w["a"])),
    ...
    body = base_flat(
        sp,
        lanes=cut(sp, sp["a"](paint("accent", 0.16, 0.12, *ramp(*w["a"])))
...
def systemshots(name, groups, sizes):
    ...
    with open(os.path.join(app, "Info.plist"), "w") as f:
        f.write('<?xml version="1.0" encoding="UTF-8"?><!DOCTYPE plist PUBLIC '
                ...
                '<key>CFBundleIdentifier</key><string>io.akira.unifieddev.shine.%s</string>'
...
HEAD = """<header><h1>Unified Dev, ten shine studies</h1>
...
def sheet(outdir):
    ...
    html = ("<!doctype html><meta charset=utf-8><title>Unified Dev icon, ten shine "
...
def sheet1024(outdir):
    ...
    html = ('<!doctype html><meta charset=utf-8><title>Unified Dev icon, ten shine '
            ...
            '<header><h1>Unified Dev, ten shine studies at 1024</h1>'
