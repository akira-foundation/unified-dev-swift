...
def unifieddevdisc(cx, cy, r, colour, alpha):
...
def base_layers(sp, mark_extra="", bleed_extra="", panel_extra="",
    ...
    if lanes is None:
        lanes = sp["a"](sheen("accent")) + sp["b"](sheen("current"))
    return [
...
def base_flat(sp, seat=1.0, panel_seat=1.0, lanes=None, bar=None,
    ...
    lanes = lanes if lanes is not None else (a(sheen("accent"))
                                             + b(sheen("current")))
...
def v04_spill(small=False):
    ...
    Two soft Unified Dev discs, one on each crossing, drawn into the Bleed group so
    ...
    def spill(k):
        return clipped("".join(
            unifieddevdisc(x, y, 210 * S, "unifieddev", 0.34 * k) for x, y in xs),
            sp["panel"])
...
def v05_filament(small=False):
    ...
    A wide Unified Dev haze hugging the bar, clipped to the panel so the spill stays
    ...
    def glow(k):
        return (haze(sp["c"], "unifieddev", 34 * S, 0.42 * k, window=sp["panel"])
...
def v08_travel(small=False):
    ...
    """
    sp = shapes(small)
    lanes = (sp["a"](axial("accent", 120, 0, 900, 0, -0.34, 0.10))
...
def v10_stack(small=False):
    ...
    sp = shapes(small)
    p, c, a, b = sp["panel"], sp["bar"], sp["a"], sp["b"]
    lanes = (a(axial("accent", 120, 0, 900, 0, -0.30, 0.08))
...
VARIANTS = [
    ...
    ("03-extrude", "the bar given a side along its whole length", "baked",
     v03_extrude),
    ("04-spill", "Unified Dev light leaking from the two crossings", "baked",
     v04_spill),
...
NOTE = {
    ...
    "04-spill": ("Two Unified Dev discs on the crossings, in the Bleed group under "
                 "the bar. Depth lives in the paint; the layered dose is raised "
                 "because the system's Mark shadow eats a third of it."),
    "05-filament": ("A wide Unified Dev haze on the bar, clipped to the panel, with a "
...
def sheet(outdir):
    ...
    html = ("<!doctype html><meta charset=utf-8><title>Unified Dev icon, ten depth "
            "studies</title><style>%s\n%s</style>"
            "<header><h1>Unified Dev, ten depth studies</h1>"
