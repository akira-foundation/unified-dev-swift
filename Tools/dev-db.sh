#!/bin/zsh
# Puts a copy of the owner's real database into Unified Dev (Dev)'s own container, so
...
# first act is to refuse while any process is running from /Applications/UnifiedDev.app.
#
...
# attaches `unifieddev.sqlite-shm` and may run recovery, and SQLite is entitled to
...
#
# COPYING unifieddev.sqlite ALONE GIVES A STALE DATABASE.
#
# In WAL mode the main file is only brought up to date by a checkpoint. Between
# checkpoints, everything committed since the last one lives in `unifieddev.sqlite-wal`
...
running=($(ud_app_pids "$UD_DEV_APP"))
if (( ${#running} > 0 )); then
  print -ru2 -- "==> Unified Dev (Dev) is running as pid ${running[*]}."
...
while true; do
  ...
  if (( attempt >= 5 )); then
    print -ru2 -- "==> gave up after $attempt attempts. Quit Unified Dev and try again."
    exit 1
  fi
  sleep 1
done
...
# apps that both believe they manage ~/unifieddev/workspaces/some-branch can both
...
# and pointing Unified Dev (Dev) at the real Unified Dev checkout is the entire reason this
...
==> --keep-paths: the copied rows point at the REAL worktrees.
    ...
    Unified Dev (Dev) and Unified Dev now both believe they manage the same directories.
    Archiving a workspace in either one deletes the other one's worktree and
    its branch. Read in the dev copy, do not act in it.

EOF
else
  /usr/bin/python3 - "$DEST" "$UD_DEV_WORKSPACES_ROOT" <<'PY'
import os
import sqlite3
import sys
