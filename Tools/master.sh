#!/bin/zsh
# Builds a copy of Unified Dev for the owner to actually use, and installs it to
# /Applications/UnifiedDev.app.
#
...
#
# Which is why it refuses to run from inside the copy it would replace. Unified Dev is
# developed in Unified Dev now, so this script can be started by an agent whose own
...
# the end.
BUILD_LOG=/tmp/unifieddev-master-build.log
build_in_worktree() {
...
build_in_worktree swift build -c release --product UnifiedDev --scratch-path /tmp/unifieddev-master-build
...
/usr/bin/defaults write "$DEST/Contents/Info.plist" MasterCommit -string "$RESOLVED" 2>/dev/null || true
...
if [ "$LAUNCH" -eq 1 ]; then
  ...
  pkill -f "$DEST/Contents/MacOS/UnifiedDev" 2>/dev/null || true
  sleep 1
  open "$DEST"
  echo "==> launched"
fi
