...
# Pass --review-fold-only to check that marking a file as viewed folds it, and that a resize leaves the folding alone.
set -euo pipefail
cd "$(dirname "$0")/.."

bin_dir="$(swift build --show-bin-path)"
probe_root="$(mktemp -d "${TMPDIR:-/tmp}/unifieddev-review-probe.XXXXXX")"
probe_app="$probe_root/Unified Dev Review Probe.app"
...
/usr/libexec/PlistBuddy -c 'Set :CFBundleIdentifier io.akira.unifieddev.review-probe' "$probe_app/Contents/Info.plist"
/usr/libexec/PlistBuddy -c 'Set :CFBundleName Unified Dev Review Probe' "$probe_app/Contents/Info.plist"
...
if not json.loads(report)['passed']:
    if os.environ.get('RUNNER_TEMP'):
        evidence = pathlib.Path(os.environ['RUNNER_TEMP'], 'unifieddev-review-probe')
