#!/bin/zsh
# Wraps a built UnifiedDev.app in the beach disk image, dist/Unified Dev-<version>.dmg.
#
...
if [ -z "$APP" ]; then
  for candidate in .build/release/UnifiedDev.app \
                   .build/arm64-apple-macosx/release/UnifiedDev.app \
                   .build/arm64-apple-macosx/debug/UnifiedDev.app; do
    if [ -d "$candidate" ]; then APP=$candidate; break; fi
  done
fi
if [ -z "$APP" ] || [ ! -d "$APP" ]; then
  echo "no UnifiedDev.app under .build; run make app first, or pass --app <path>" >&2
  exit 1
fi
...
[ -n "$OUT" ] || OUT="$PWD/dist/Unified Dev-$version.dmg"
...
hdiutil detach /Volumes/Unified Dev >/dev/null 2>&1 || true
rm -f "$WORK/Unified Dev.dmg" "$WORK/rw.dmg" "$OUT"
...
if ! "$VENV/bin/dmgbuild" \
  ...
  Unified Dev "$WORK/Unified Dev.dmg" >"$WORK/dmgbuild.log" 2>&1; then
...
# stays connected.
hdiutil convert "$WORK/Unified Dev.dmg" -format UDRW -o "$WORK/rw.dmg" >/dev/null
...
rm -f "$WORK/Unified Dev.dmg" "$WORK/rw.dmg"
