...
# allowed to update itself. `BuildChannel` is what says which of the two this is;
...
# MasterCommit is checked as a second, independent guard on the same case.
#
#   UD_VERSION=0.2.0 UD_BUILD=7 ./Tools/build.sh -r
#
# UD_BUILD has to increase with every release and never repeat. The release pipeline should
# derive both from the tag it is building, and the appcast's sparkle:version MUST equal the
# UD_BUILD of the zip it points at, or an installed app updates to a build that still reports
...
#
#   UD_UPDATE_FEED_URL     the appcast, e.g. https://<bucket-host>/appcast.xml
#   UD_UPDATE_PUBLIC_KEY   the base64 EdDSA public key from Sparkle's generate_keys
#
...
if [[ -n "${UD_UPDATE_FEED_URL:-}" ]]; then
  plist_set SUFeedURL string "$UD_UPDATE_FEED_URL"
fi
if [[ -n "${UD_UPDATE_PUBLIC_KEY:-}" ]]; then
  plist_set SUPublicEDKey string "$UD_UPDATE_PUBLIC_KEY"
fi
...
embed_sparkle() {
  ...
  /usr/bin/install_name_tool -add_rpath "@executable_path/../Frameworks" "$APP/Contents/MacOS/UnifiedDev" 2>/dev/null
  ...
  if ! otool -l "$APP/Contents/MacOS/UnifiedDev" | grep -q '@executable_path/../Frameworks'; then
