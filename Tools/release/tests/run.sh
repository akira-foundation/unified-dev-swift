...
ns = {
    ...
    "unifieddev": "https://unified-dev.akira-io.com/xml-namespaces/unifieddev",
}
...
"$TOOLS/appcast.py" --existing "$WORK/does-not-exist.xml" --output "$FEED" \
  --version 1.0.0 --build 100 --url https://example.invalid/Unified Dev-1.0.0.zip \
...
# A newer one, on top.
"$TOOLS/appcast.py" --existing "$FEED" --output "$FEED" \
  --version 1.1.0 --build 110 --url https://example.invalid/Unified Dev-1.1.0.zip \
...
"$TOOLS/appcast.py" --existing "$FEED" --output "$FEED" \
  --version 1.0.1 --build 105 --url https://example.invalid/Unified Dev-1.0.1.zip \
...
# A prerelease.
"$TOOLS/appcast.py" --existing "$FEED" --output "$FEED" \
  --version 1.2.0-beta.1 --build 120 --url https://example.invalid/Unified Dev-1.2.0-beta.1.zip \
...
"$TOOLS/appcast.py" --existing "$FEED" --output "$FEED" \
  --version 1.3.0 --build 130 --url https://example.invalid/Unified Dev-1.3.0.zip \
...
"$TOOLS/appcast.py" --existing "$FEED" --output "$FEED" \
  --version 1.4.0 --build 140 --url https://example.invalid/Unified Dev-1.4.0.zip \
...
"$TOOLS/appcast.py" --existing "$WORK/existing.xml" --output "$WORK/merged.xml" \
  --version 1.3.0 --build 300 --url https://example.invalid/Unified Dev-1.3.0.zip \
...
# enclosure is what Sparkle downloads and every installed copy of Unified Dev already
...
"$TOOLS/appcast.py" --existing "$WORK/does-not-exist.xml" --output "$IMAGE_FEED" \
  --version 2.0.0 --build 200 --url https://example.invalid/Unified Dev-2.0.0.zip \
  --length 4096 --signature SIGDMG --min-system 15.0 \
  --dmg-url https://example.invalid/Unified Dev-2.0.0.dmg --dmg-length 123456 2>/dev/null

expect_equal "the disk image is named on the item" \
  "$(xpath_attr "$IMAGE_FEED" 'channel/item/unifieddev:diskImage' 'url')" \
  "https://example.invalid/Unified Dev-2.0.0.dmg"
expect_equal "the disk image carries its length" \
  "$(xpath_attr "$IMAGE_FEED" 'channel/item/unifieddev:diskImage' 'length')" "123456"
expect_equal "the disk image says what it is" \
  "$(xpath_attr "$IMAGE_FEED" 'channel/item/unifieddev:diskImage' 'type')" "application/x-apple-diskimage"
expect_equal "the enclosure is still the zip" \
  "$(xpath_attr "$IMAGE_FEED" 'channel/item/enclosure' 'url')" \
  "https://example.invalid/Unified Dev-2.0.0.zip"
...
expect_equal "a release without an image has no diskImage element" \
  "$(xpath_attr "$FEED" 'channel/item/unifieddev:diskImage' 'url')" ""
...
if BIN="$("$TOOLS/sparkle-tools.sh" 2>/dev/null)"; then
  echo "hello unifieddev" > "$WORK/payload.zip"
