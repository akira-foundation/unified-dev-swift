...
#
#   UD_BUCKET            bucket name
#   UD_S3_ENDPOINT       https endpoint of the region the bucket is in
#   UD_S3_REGION         region name used to sign the request
#   UD_PUBLIC_BASE_URL   what a browser uses to reach the same objects
#   AWS_ACCESS_KEY_ID       from the environment, never on the command line
#   AWS_SECRET_ACCESS_KEY
#   UD_BUCKET_PREFIX     optional key prefix, no leading or trailing slash
#   UD_S3_ACL            optional canned ACL. Leave it unset and make the
...
: "${UD_BUCKET:?bucket.sh: UD_BUCKET is not set}"
: "${UD_S3_ENDPOINT:?bucket.sh: UD_S3_ENDPOINT is not set}"
: "${UD_S3_REGION:?bucket.sh: UD_S3_REGION is not set}"
: "${UD_PUBLIC_BASE_URL:?bucket.sh: UD_PUBLIC_BASE_URL is not set}"

PREFIX=${UD_BUCKET_PREFIX:-}
PREFIX=${PREFIX#/}
PREFIX=${PREFIX%/}
...
public_url() {
  echo "${UD_PUBLIC_BASE_URL%/}/$(remote_key "$1")"
}
...
case "$ACTION" in
  ...
  put)
    ...
    [ -n "${UD_S3_ACL:-}" ] && ARGS+=(--acl "$UD_S3_ACL")
