...
#
#   UD_CERT_P12_BASE64     base64 of the exported .p12
#   UD_CERT_P12_PASSWORD   the password it was exported with
#
...
if [ -z "${CI:-}" ] && [ -z "${UD_ALLOW_KEYCHAIN_OUTSIDE_CI:-}" ]; then
...
KEYCHAIN="${RUNNER_TEMP:-${TMPDIR:-/tmp}}/unifieddev-signing.keychain-db"
SEARCH_LIST_BACKUP="${RUNNER_TEMP:-${TMPDIR:-/tmp}}/unifieddev-keychain-search-list"

import_certificate() {
  : "${UD_CERT_P12_BASE64:?keychain.sh: UD_CERT_P12_BASE64 is not set}"
  : "${UD_CERT_P12_PASSWORD:?keychain.sh: UD_CERT_P12_PASSWORD is not set}"
  ...
  umask 077
  certificate="$(mktemp -t unifieddev-cert)"
  printf '%s' "$UD_CERT_P12_BASE64" | base64 --decode > "$certificate"
  ...
  security import "$certificate" -k "$KEYCHAIN" -P "$UD_CERT_P12_PASSWORD" \
