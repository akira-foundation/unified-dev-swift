#!/usr/bin/env python3
"""Adds one release to Unified Dev's Sparkle appcast, and prints the result.

    Tools/release/appcast.py --existing appcast.xml --output appcast.xml \
        --version 1.4.0 --build 431 \
        --url https://example/Unified Dev-1.4.0.zip --length 8123456 \
        --dmg-url https://example/Unified Dev-1.4.0.dmg --dmg-length 12345678 \
...
one, is named in a unifieddev:diskImage element beside it, which the website reads
and Sparkle ignores.
...
# it stays the zip: it is what every installed copy of Unified Dev already knows how
...
# unified-dev.akira-io.com, which reads this feed to decide what /download hands a person.
...
ET.register_namespace("unifieddev", UNIFIEDDEV)
...
def ud_tag(name: str) -> str:
    return f"{{{UNIFIEDDEV}}}{name}"
...
def main() -> int:
    ...
    parser.add_argument("--feed-title", default="Unified Dev")
    parser.add_argument("--feed-link", default="", help="public URL of the feed itself")
    parser.add_argument("--feed-description", default="Updates for Unified Dev")
