#!/bin/zsh
# Builds a Unified Dev anyone can open, and leaves both a zip and the beach disk
# image in dist/.
#
...
#      machine.
#
#   2. A stored notarytool credential named `unifieddev`.
#      Make an app-specific password at appleid.apple.com, then once:
#
#        xcrun notarytool store-credentials unifieddev \
#          --apple-id you@example.com --team-id <TEAMID> --password <app-specific>
#
# Then set the identity, once, in your shell profile:
#
#   export UD_CODESIGN_IDENTITY="Developer ID Application: Name (TEAMID)"
...
WORK=/tmp/unifieddev-release-src
OUT="$PWD/dist"
...
BUILD_LOG=/tmp/unifieddev-release-build.log
if ! ( cd "$WORK" && UD_CODESIGN_IDENTITY="${UD_CODESIGN_IDENTITY:-}" \
...
APP="$WORK/.build/release/UnifiedDev.app"
[ -d "$APP" ] || APP="$(cd "$WORK" && swift build -c release --show-bin-path)/UnifiedDev.app"

mkdir -p "$OUT"
ZIP="$OUT/Unified Dev-$version.zip"
DMG="$OUT/Unified Dev-$version.dmg"
