...
///
/// An intent is performed inside Unified Dev's own process, but not necessarily inside a Unified Dev that has
...
/// intent is one more caller of Unified Dev's existing entry points either way, never a second owner of
/// its state.
@MainActor
enum RunningApp {
    /// The state the window is running on, handed over by `AppDelegate` once the scene
    /// exists.
    ///
    /// Weak and explicit, the same bargain `ServicesProvider`, `SwitchProbe` and
    ...
    ///
    /// This replaced a `unifieddev://` link and a poll. An intent used to build a URL, hand it to the
    ...
    /// Hands the app a `unifieddev://` link, which is the entry point it already uses for creating a
