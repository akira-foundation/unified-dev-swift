...
enum WorkspaceLookup {
    ...
    /// outside the app is the session row the runner marks `running`. Unified Dev resets those rows on
    ...
    /// somebody removed from under Unified Dev. None of those is worth failing an intent over, so the
