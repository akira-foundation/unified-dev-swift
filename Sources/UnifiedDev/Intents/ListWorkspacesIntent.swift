...
struct ListWorkspacesIntent: AppIntent {
    ...
    static let description = IntentDescription(
        """
        Returns Unified Dev's workspaces with their status and diff size. Reads Unified Dev's database \
        directly, so it answers whether or not Unified Dev is open.
        """,
    ...
    static var parameterSummary: some ParameterSummary {
        Summary("Get Unified Dev workspaces") {
            \.$onlyRunning
            \.$includePullRequests
        }
    }
