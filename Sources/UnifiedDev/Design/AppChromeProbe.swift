...
enum AppChromeProbe {
    ...
    static func runAndExit() -> Never {
        guard Bundle.main.bundleIdentifier != "io.akira.unifieddev",
              ProcessInfo.processInfo.environment["UD_DB_PATH"] != nil else { exit(1) }
    ...
    private static func render(_ content: some View, size: CGSize, name: String) async -> Bool {
        ...
        try? bitmap.representation(using: .png, properties: [:])?.write(to: URL(filePath: "/tmp/unifieddev-chrome-\(name).png"))
        return aligned
    }
}
