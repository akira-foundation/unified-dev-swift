...
enum ArchiveFailureProbe {
    ...
    private static func checkBridge(app: AppModel, store: Store, root: URL) async throws -> [String] {
        ...
        try await Shell.check("git", ["-c", "user.name=Unified Dev Probe", "-c", "user.email=probe@unifieddev.local",
    ...
    private static func run() async {
        guard Bundle.main.bundleIdentifier?.hasPrefix("io.akira.unifieddev.typography-") == true else {
        ...
        do {
            let root = FileManager.default.temporaryDirectory
                .appendingPathComponent("unifieddev-archive-probe-\(UUID().uuidString)")
