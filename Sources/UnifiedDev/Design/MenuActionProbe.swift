...
/// after.
///
///     Unified Dev --menu-action "Project settings…" [--menu-action-row 1]
///     Unified Dev --menu-action "Project Settings…" --menu-action-scope main
///     Unified Dev --menu-action "Project settings…" --menu-action-close commandW|escape
///     Unified Dev --menu-action "Project settings…" --menu-action-reopen
///     Unified Dev --menu-action "Project settings…" --menu-action-hide miniaturize|orderOut
///
/// **It exists because "the window did not open" is the one report the test suite cannot answer.**
/// `Tests/CoreTests` links the core alone, so nothing there knows a scene exists;
...
///
/// Debug builds only, and it wants `UD_DB_PATH` pointed at a scratch database with a project in
