...
///
///     Unified Dev --menu-probe /tmp/menu.png [--menu-part menu|kinds|terminal|terminalKinds|browser|row|colour|worktree|project|projectHidden|filter|diffScope] [--menu-project <path>] [--menu-base <branch>]
///
...
/// through a real `AppTerminalView`'s right mouse handling rather than through `NSMenu.popUp`,
...
/// want `UD_DB_PATH` set at a seeded scratch copy rather than the owner's.
///
...
enum MenuProbe {
    ...
    private enum Part: String {
        ...
        case terminalKinds
        /// A browser page's own menu, which is WebKit's and not Unified Dev's.
        case browser
    ...
    /// it up, through `AppTerminalView`, because AppKit adds items of its own on that path and
