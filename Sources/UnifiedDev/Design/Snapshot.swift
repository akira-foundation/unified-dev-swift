...
///
///     Unified Dev --snapshot /tmp/shots
@MainActor
enum Snapshot {
    ...
    /// no such button, and the button had been in the source the whole time. `.build/debug/Unified Dev`
    ...
    /// which is `.build/<configuration>/Unified Dev` in a checkout and nothing at all in a shipped copy.
    ...
    private static var packageRoot: URL? {
        ...
        // .build/<configuration>/Unified Dev, so three levels up is the package.
        let root = executable
    ...
    /// Opens a `unifieddev://` URL in THIS process, a few seconds after launch.
    ///
    /// `open unifieddev://...` from a shell goes through LaunchServices, which picks whichever copy of
    /// the app it feels like and does not carry `UD_DB_PATH`, so a test that drives a deep link
    ...
    ///
    ///     Unified Dev --open-url "unifieddev://prompt=...&path=..."
    ...
    ///
    ///     Unified Dev --select w1 --expand-setup-log 8
    ///
    ...
    ///
    ///     Unified Dev --terminal-workspace plumage
    ///
    ...
    ///
    ///     Unified Dev --notice 4 "Unified Dev named this workspace X. Its branch is still `y`, because ..."
    ///
    ...
    /// Unified Dev did something it did not do.
    ...
    ///
    ///     Unified Dev --select w1 --running w1,w2,w3
    ///     Unified Dev --select w1 --running w1,w2,w3/w1/
    ///
    ...
    ///
    ///     Unified Dev --snapshot-window /tmp/shots/window.png [--window-size 900x700]
    ...
    static func scheduleWindowCapture() {
        ...
        Task { @MainActor in
            ...
            // comparison against "Unified Dev" matched neither and every settings capture silently
    ...
    ///
    ///     Unified Dev --snapshot-gallery /tmp/shots [--gallery review-comments|inspector-tabs|diff-scope]
    ///
    ...
    /// item is in the same position: `AppCommands` replaces the `.appInfo` group, so the action
    ...
    /// to start without `UD_DB_PATH`, so the `bootstrap` here can never touch the user's real
    /// workspaces.
...
private struct ComponentGallery: View {
    var body: some View {
        VStack(alignment: .leading, spacing: 18) {
            ...
            // Task 7 report: both of these used to size and outline themselves by hand before
            // their button style ever ran, which is the fault this gallery exists to keep from
            // coming back unnoticed. Both are now nothing but standard modifiers.
