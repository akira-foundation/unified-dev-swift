...
/// app's own `SidebarNavRow` on the list's own native selection fill, with the table made first
/// responder by hand and with a top level row and a workspace row selected at once so both fills
/// are in one photograph. Multiple selection is not a state the pane itself offers; it is here
/// only so that the two rows can be compared without flicking between two files.
///
...
///
///     Unified Dev --snapshot-gallery <dir> --gallery sidebar-selection
