...
/// across. The decisions are `ContinuationGate` and `CarryOnGate` in Core, and they are the
...
extension AppModel {
    ...
    func startWorkspace(
        ...
        /// Overrides the name Unified Dev would derive. Only the bridge passes one; the sheet lets the
        /// namer do its job.
        name: String? = nil,
        ...
        // fallback. The sheet and the bridge both pass controls; a `unifieddev://` link, the Services
        ...
        let request = WorkspaceStartRequest(
            ...
            repo: repo,
            prompt: spoken,
            // Almost always the owner: the sheet, a `unifieddev://` link, the Services menu and a
        ...
        // Said out loud, because this is a preference the owner set that Unified Dev has just undone,
        // and the workspace that undid it can have been asked for by an agent on the bridge with
        // nobody watching. A banner is exactly what `Notice` is for: something the app did
    ...
    /// standing in front of the strip. A refusal is Unified Dev declining on purpose and naming the
    ...
    /// `ContinuationGate.decide` in Core and is the only thing here allowed to say yes; the
    ...
    /// come across is Unified Dev's own transcript, whose rows belong to the chat they were written
