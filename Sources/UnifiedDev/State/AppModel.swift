...
final class AppModel {
    ...
    var alert: AppAlert?
    /// The corner notice. See `Notice`. Set from anywhere, drawn once by `RootView`.
    ///
    ...
    /// The conversation that belongs to Unified Dev rather than to a workspace. See `AskModel`.
    ///
    ...
    /// The mark the Ask Unified Dev row wears, or nil when it has nothing to say.
    ///
    ...
    /// started. The report was the rule on Ask Unified Dev's own pane sitting still through a whole
    ...
    /// set here rather than in an initialiser, because `UnifiedDevApp` builds this as `@State` and
    ...
    func bootstrap() async {
        ...
        do {
            ...
            // closed here and the rows that asked them say what happened instead. Unified Dev denies
    ...
    ///
    /// The entry names an absolute path and is written once, so dragging Unified Dev from
    ...
    /// Quitting Unified Dev has to take everything it started with it. macOS does not kill a process's
    ...
    func shutdownEverything() async {
        ...
        // one after another. Ask Unified Dev is signalled with them, and only if it was ever opened:
    ...
    /// the runner's event pump. Unified Dev has already put out two performance fires, a busy indicator
    ...
    /// when the user edits files outside Unified Dev.
    ...
    private func refreshSelectedChangedFiles() async {
        ...
        // A worktree removed outside Unified Dev would make git walk up to the parent repository and
    ...
    func refreshDiffStats() async -> Set<WorkspaceID> {
        ...
        // seconds on an idle machine is what put Unified Dev under "Using Significant Energy".
        //
        ...
        let pending = workspaces.filter {
            // A worktree that has been removed outside Unified Dev would make git walk up to the parent
    ...
    /// width now depends on it and `UnifiedDevApp` cannot see a private computed property on a view.
    ...
    /// back to "Unified Dev". The Workspace menu no longer asks: what a menu item acts on is
    ...
    private func recomputeAgentTurns() {
        ...
        // Ask Unified Dev is in neither set, because a `SessionActivity` carries a workspace id and
    ...
    var runningAgentCount: Int {
        // Ask Unified Dev counts, and only if it has been opened this launch. It is an agent mid turn
    ...
    /// `SidebarReorder` is what turns them into the store's own order, and it lives in `Core`
