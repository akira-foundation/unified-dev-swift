...
final class TranscriptModel {
    ...
    ///
    /// **Nil is Ask Unified Dev and nothing else today.** Its id and path are stable and are what most
    ...
    /// New Ask conversations use the folder chosen in Settings, with Unified Dev's own folder as fallback.
    let cwd: String
    ...
    /// the queue, because a message somebody typed has to survive quitting Unified Dev. See `Delivery`.
    ...
    /// The conversation that belongs to Unified Dev rather than to a workspace.
    ///
    ...
    /// into `messages` for the reason `WorkspaceEvent`'s header sets out: a row Unified Dev writes about
    ...
    /// payload are not. Guarded by `isRunning` at the call site so a turn Unified Dev sent, whose `init`
    ...
    private func ensureRunner() -> (any SessionRunner)? {
        ...
        // minted for that workspace and the role its origin says; Ask Unified Dev gets the owner's own,
    ...
    /// the probe has to measure is what Unified Dev does with a delta, and the only faithful way to
    ...
    private func handle(_ event: AgentEvent) async {
        switch event {
        case .initialized:
            // **A turn Unified Dev did not send starts here and nowhere else.** The CLI runs a
            // background task's completion notification as a turn of its own, and it announces
            // each one with an `init` exactly as it announces the ones Unified Dev asks for. Until this
    ...
    /// "Always allow ... in Unified Dev" is a promise about a place, and the place has to be named.
    ...
    /// to ask either polls or forgets, so the end of a crew member's turn is an event Unified Dev
