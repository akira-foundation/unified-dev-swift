...
/// in `NotificationPolicy` and `NotificationDigest`, in Core, where it can be tested without
...
final class NotificationService {
    ...
    /// a morning of crash reports named Unified Dev: `.build/debug/Unified Dev` run as a bare executable died
    ...
    /// permission in System Settings while Unified Dev is running, and a toggle that keeps claiming to
    ...
    /// Handed the app state once it exists, the same way `AppDelegate` is. The suppression
    ...
    /// Whether macOS is currently throwing away everything Unified Dev sends. Drives the warning in the
    /// settings pane.
    ...
    func openSystemSettings() {
        // The Ventura-and-later pane id. The `id` query selects Unified Dev's own row, so the switch the
    ...
    /// Watches CI on the pull requests Unified Dev already knows about.
    ///
