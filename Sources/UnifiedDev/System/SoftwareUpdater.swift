...
/// Unified Dev's half of Sparkle: when it may look for a new version, and what has to be true before it
...
final class SoftwareUpdater: NSObject, SPUUpdaterDelegate {
    ...
    /// proxy to `AppDelegate` silently fails, leaving the quit confirmation enabled during
    ...
    ///
    /// This is the last callback before Unified Dev is terminated and replaced, so it is the only place
