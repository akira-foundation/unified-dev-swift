...
/// Reads back what this running copy of Unified Dev told its own log, so a feedback report can carry it.
///
...
/// belongs in a bug report is what this launch of Unified Dev did, not what the machine did. Nothing
/// here can see another application's log, the system's log, or a previous run of Unified Dev.
///
/// **Unified Dev's own lines, and no framework's.** The predicate keeps only entries whose subsystem is
/// Unified Dev's, so the URL loading system, AppKit, Sparkle and everything else that logs from inside
