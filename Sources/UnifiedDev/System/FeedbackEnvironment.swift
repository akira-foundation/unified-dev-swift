...
enum FeedbackEnvironment {
    ...
    /// The scale of the screen Unified Dev is being read on. `2` on any Retina display.
    static func displayScale() -> Double {
        Double(NSScreen.main?.backingScaleFactor ?? 1)
    }

    /// The language tag Unified Dev is being read under: `nl-BE`, `en`.
    ///
    ...
    /// how this copy of Unified Dev is set up, and the mode a single session was switched into is that
    /// session's business.
