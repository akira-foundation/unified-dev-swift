...
/// `WelcomePromptSubmission`. Neither of the last two can be
/// made empty by anything about this Mac, so neither is ever left out.
///
/// Three bands, in the register the About window established: the plinth, the reading ground
/// under a hairline, and a chrome strip at the foot with the buttons in it. Nothing here is a new
/// surface and nothing here is a new colour; the only thing this window adds to the app's
/// vocabulary is the line down the left of the checks, and that line is carrying information
/// rather than decorating the column. See `soundingLine`.
struct WelcomeView: View {
    ...
    /// already been told about this Unified Dev.
    ...
    ///
    /// `command` is nil only if this copy of Unified Dev lost its bridge while somebody was standing on
    ...
    /// The greeting's plinth, compressed into a band. Same surface, same wordmark face, and the
    /// mark read out of the running bundle rather than shipped again.
    ///
    ...
    private var plinth: some View {
        ...
        // band is a fixed-size window's header holding display type, laid out against the
        // title bar rather than against a row of controls, and the scale is for gaps between
        // things that sit in a resizable pane. `AboutWindow` says the same about its own two. Off
        // the scale and said so, rather than off the scale in silence.
    ...
    /// the missing one. The fact worth reading first about a tool Unified Dev does not need is not that
    ...
    /// border colour below that, so a glance at the left edge says how far down the list Unified Dev has
    ...
    /// Check again, because Unified Dev cannot install Homebrew packages on somebody's behalf and
    ...
    /// in front of it, and the one-time code on screen is its output, never Unified Dev's to store, log
    ...
    private func startLogin(_ check: SetupCheck, fix: SetupFix) {
        ...
        guard let session = LoginTerminalSession(
            ...
            // Not the home directory. This is the first launch, before Unified Dev has been shown a
            ...
            // ask about it in Unified Dev's name. See `AgentScratchDirectory`.
