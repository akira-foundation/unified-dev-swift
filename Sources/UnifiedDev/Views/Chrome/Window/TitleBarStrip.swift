...
/// `WindowToolbar`'s flexible spacer, so a frame set in one step moved the field 379 points
...
/// a workspace with a state in it that changes without anybody touching Unified Dev, so it took the top
...
///
/// ## Task 7 report: why this stays a title bar accessory and does not become toolbar items
///
/// The search glyph and the inspector's toggle used to live in this same `HStack`, ahead of the
/// band. Both have moved: search is `.searchable` on the detail column now, and the toggle is a
/// trailing item in `WindowToolbar`. Both fit as toolbar items because a toolbar item is sized by
/// its own content and neither cared what anything else on screen measured.
///
/// The band cannot follow them, and that is a fact about `NSToolbarItem`, not a preference. A
/// toolbar item has no way to be told "be exactly as wide as the inspector pane, and slide with
/// it": `NSToolbar` sizes every item from its view's own intrinsic size or a fixed width set once,
/// and nothing here is either. The pane's width changes on every frame of a drag and of the
/// collapse animation, `InspectorGeometry` exists only to carry that number to something that is
/// not a view in the split, and a title bar accessory is the one AppKit container whose frame this
/// code is allowed to set by hand, every frame, from outside. So the band is what is left in the
/// accessory, and the accessory is now exactly as wide as the band it draws, `controls` in
/// `TitleBarStripController.resize` gone with the two items it used to hold room for.
...
final class TitleBarStripController: NSTitlebarAccessoryViewController {
    ...
    /// out in what the trailing accessory leaves it, and `WindowToolbar`'s flexible spacer
