...
///
/// Unified Dev's Cmd+W closes the tab, deliberately: the window lists every workspace and every agent in
...
/// carry. `AppCommands` retired Close Session: a conversation IS a tab in the centre strip, and
/// the old item closed the workspace's active chat whatever was actually in front of you.
///
/// It is done in AppKit rather than in `AppCommands` because SwiftUI has no command group that
...
/// group. Adding a second Close of Unified Dev's own would have meant two items called Close in one
