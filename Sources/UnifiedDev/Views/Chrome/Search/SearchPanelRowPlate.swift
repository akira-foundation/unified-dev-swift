...
extension View {
    ...
    ///
    /// Never focused, on purpose, and that is a change: it forced `isFocused: true` on the
    /// argument that the panel's field really does hold the keyboard while the arrows walk this
    /// list, which is true, but the fill that answer draws is `Palette.selectedEmphasized`, the
    /// system's own full-strength accent plate. Photographed, that plate is the brand purple at
    /// full saturation sitting under a whole row, which is exactly the "violent purple band" the
    /// owner reported. `SidebarView.selectionFill` answers the same question, a row selected while
    /// something else legitimately holds the keyboard, with one quiet shade regardless of focus,
    /// and this plate now follows it instead of AppKit's own emphasis rule.
