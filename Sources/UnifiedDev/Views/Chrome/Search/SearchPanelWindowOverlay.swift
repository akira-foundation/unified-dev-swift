...
/// has far less to take out of Unified Dev's dark ramp than out of its light one, so one opacity read as
...
///
/// Shared rather than passed, and for the reason `InspectorGeometry` next door is: Unified Dev is one
...
/// picture. `contentLayoutRect` is what the title bar leaves over AFTER its accessories, and Unified Dev
...
final class SearchPanelWindowGeometry {
    ...
    /// leaves over AFTER its accessories, and Unified Dev puts a 52 point band up there, so it answered
