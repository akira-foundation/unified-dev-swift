...
/// a system service does and Unified Dev is not one. See `SearchPanelWindowOverlay` for the dimming, the
                ...
                LazyVStack(alignment: .leading, spacing: SearchPanelRowMetrics.gap) {
                    ForEach(panel.listing.sections) { section in
                        if let title = section.title {
                            ...
                            //
                            // Title-style capitalisation, not all caps: a macOS 26 system change,
                            // per Task 7's report, and `title` already reads that way ("Workspaces",
                            // "Recently open"). No `.tracking` either, which was set for capitals
                            // sitting nearly solid at ten points and has nothing to do here.
                            Text(title)
                                .font(Typo.micro)
                                .foregroundStyle(Palette.textTertiary)
                                .padding(.horizontal, Metrics.inset)
