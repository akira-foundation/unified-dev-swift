...
///
/// `AppCommands` is one body, shared by every window and every screen the app has, so an item
...
extension FocusedValues {
    ...
    /// Command-Backspace deletes to the start of the line in every text field on macOS, and Unified Dev
    /// had given it to Archive Workspace. A user typing a prompt reached for it, and archived the
    /// workspace he was writing in: "in Unified Dev this triggers a shortcut to delete the workspace".
    ///
    ...
    /// What each of Home's scope chips would show, published by `HomeView` so the toolbar item
    /// that replaced `HomeBar`'s segmented control can print the same badge without repeating the
    /// pass over every workspace that `HomeList.build` already made. See `WindowToolbar`.
