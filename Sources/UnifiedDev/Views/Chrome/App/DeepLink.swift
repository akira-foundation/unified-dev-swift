...
/// Preserves Conductor deep links so existing scripts can hand work to Unified Dev unchanged.
@MainActor
enum DeepLink {
    /// The schemes this copy actually answers to, read off its own bundle rather than written out.
    ///
    /// It was the literal `"unifieddev"`, and `Tools/dev-build.sh` rebrands the registered scheme to
    /// `unifieddevdev` so the dev copy cannot swallow a link meant for the real one. The result was a
    /// dev build that registered `unifieddevdev://`, received them, and then refused every one with
