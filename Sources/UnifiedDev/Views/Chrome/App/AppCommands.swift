...
struct AppCommands: Commands {
    ...
    var body: some Commands {
        // Unified Dev's own About window rather than AppKit's panel. `orderFrontStandardAboutPanel` can
        // be handed the bundle's keys and a paragraph of credits and nothing else: no mark, no
        // typeface, no ground. Everything that makes this app recognisable as the one unified-dev.akira-io.com
        ...
        // Directly under "About Unified Dev", which is where every Mac app that updates itself puts it
        // and therefore the first place anyone looks. Not in the Help menu, and not in Settings
        // only: the Settings switch decides whether Unified Dev looks on its own, and this asks now.
        //
        ...
        CommandGroup(replacing: .newItem) {
            ...
            // Reached from the menu as well as from the toolbar glyph that appears while Ask Unified Dev
            // is open. See the note beside this item in `MenuBarCatalogue`: the toolbar button is
            // a control you have to be looking at Ask Unified Dev to see, so it could not answer
            ...
            // Shift+Cmd+N, which nothing in Unified Dev held. It is the initial of the thing, which is
    ...
    /// keyboard.
    ///
    /// Unified Dev bound neither these nor Ctrl+Tab: the full shortcut grep found 74 bindings and not one
