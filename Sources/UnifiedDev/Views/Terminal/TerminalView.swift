...
final class AppTerminalView: LocalProcessTerminalView {
    ...
    /// Set when Unified Dev is the one ending this shell rather than the shell ending by itself. The two
    ...
    /// everywhere else rather than at Unified Dev's own body size.
    ...
    /// Tells the terminal that Unified Dev is about to end this shell, ahead of whatever signal does it.
    ...
    fileprivate func handleProcessExit(_ status: Int32?) {
        ...
        hasExited = true
        // Unified Dev ended this one, so there is nobody to tell and nothing to close.
    ...
    /// SwiftTerm ships a palette that looks nothing like the rest of Unified Dev, so both the sixteen
    ...
    /// Whatever Ghostty says, with Ghostty's own defaults behind it. Unified Dev's colours are not
    ...
    ///
    /// Red, yellow and blue are Unified Dev's, so a failing test's red in the terminal is the same red
    ...
    /// SwiftTerm stores terminal colours as three opaque byte channels, while Unified Dev keeps dynamic
