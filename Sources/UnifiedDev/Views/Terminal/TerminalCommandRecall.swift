...
final class TerminalCommandRecall {
    ...
    /// What Unified Dev itself last sent into a pane, which is preferred over what `ps` reports.
    ///
    ...
    /// the command was `npm run dev`. So the exact text wins whenever Unified Dev has it.
    ///
    ...
    // MARK: - What Unified Dev typed

    /// A line Unified Dev put into a pane itself.
    ///
    ...
    /// what Unified Dev last sent into this pane, so the next poll records it as the pane's own text
    ...
    /// it is the wrong one to rely on alone: Unified Dev is developed in Unified Dev, it gets killed, force
