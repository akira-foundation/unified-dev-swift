...
/// The tmux server that holds shells for panes, so quitting Unified Dev no longer takes them with it.
///
...
///
/// The server runs on a private socket with a configuration Unified Dev writes itself, so the user's own
...
final class TerminalPersistence {
    ...
    /// The configuration is regenerated on every launch rather than written once, so a Unified Dev update
    ...
    /// Everything one workspace owns, read off the session names rather than off Unified Dev's own
