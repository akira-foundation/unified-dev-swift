...
final class CenterTabStore {
    ...
    /// their panes anyway: a tmux session whose tab is only on disk is still one Unified Dev can reach,
    ...
    /// down and nothing under `Sources/UnifiedDev` can be tested. It used to number from the count of
