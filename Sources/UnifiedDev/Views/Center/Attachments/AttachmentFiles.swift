...
/// keeping git blind to the ones Unified Dev made.
///
/// Nothing here touches the main actor, so a hundred megabyte copy runs where a copy belongs.
enum AttachmentFiles {
    /// The most Unified Dev will copy into a worktree for one attachment.
    ///
    ...
    /// The most Unified Dev will take off a clipboard before it has looked at it.
    ///
    ...
    /// Bytes Unified Dev made itself, written straight out under the name it chose.
    ///
    ...
    // Not here. Moving a staged draft into the worktree it was written for is
    // `WorkspaceStartAttachments.adopt`, in the core, beside the two rules about what the
    // sentence may then name. The move and the naming are one decision wearing two hats, and
    // splitting them across a target the suite cannot reach is how they came apart: the sheet
    // stripped the files out of the sentence, the move carried them in anyway, and the agent was
    // told about neither.

    /// Takes a copy back off the disk. Only ever called for a copy Unified Dev made and that has not
    ...
    /// Makes the attachments folder invisible to git, which is not a nicety: Unified Dev's whole premise
    ...
    ///
    /// The arrangement itself is `WorktreeScratch`, in Core, because attachments are not the
    /// only thing Unified Dev writes into somebody else's checkout and the rule has to be one rule. See
