...
/// Unified Dev runs is spawned with the worktree as its working directory, so a relative path inside it
...
struct PromptAttachment: Identifiable, Hashable, Codable, Sendable {
    ...
    var source: String = ""
    /// True when Unified Dev made this copy under `.unifieddev/attachments`, which is what makes removing the
    ...
    /// are the honest answers for something Unified Dev is only reading back: it is not a copy this
...
/// Where an attachment came from, before it is a file Unified Dev can point at.
///
...
enum AttachmentSource: Hashable, Sendable {
    ...
    /// Words Unified Dev generated that the agent is meant to read as a file: a CI log fetched for a
