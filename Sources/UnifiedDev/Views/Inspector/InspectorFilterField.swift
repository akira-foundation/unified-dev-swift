...
/// The strip above one of the inspector's lists that narrows it to what somebody is looking for.
///
/// **One field for both tabs of the column rather than one each.** All files and Changes are two
/// lists of the same repository's files, a segment apart, and a reader who has learnt what typing
/// here does has learnt it for the pane next door. What each tab does with the text is its own
/// (`FileTreeFilter` and `ChangedFileFilter` are different walks and say why), but the control,
/// the placeholder, the clear button and what Escape and Return mean are the same in both.
///
/// **While this field has the keyboard it owns every character, and the list's type-select does
...
struct InspectorFilterField: View {
    ...
    /// Escape. See `FileTreeView.escape`, which both lists spell the same way, for its two steps.
    var onEscape: @MainActor () -> Void
    /// Return, which hands the keyboard to the list.
    ///
    ...
    /// different one, and sending the keyboard to the list would make them click twice to do it.
    private func clear() {
        query = ""
        isFocused = true
    }
