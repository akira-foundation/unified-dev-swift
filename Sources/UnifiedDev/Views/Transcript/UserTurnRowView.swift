...
/// `.unifieddev/attachments` tells them nothing they did not know. See `AttachmentTrailer` for the one
/// place that format is written and read.
///
/// Instructions Unified Dev appended to a turn it composed itself are drawn the same way, as a chip
...
struct UserTurnRowView: View {
    ...
    /// A chip opens where every other file in Unified Dev opens, which is the review tab, and it is the
    ...
    private func open(_ path: String) {
        // No workspace is Ask Unified Dev, which has no review pane for a file to open into. The chip
    ...
    ///
    /// For a block Unified Dev put in the message there is nothing to resolve and nothing to ask the disk:
