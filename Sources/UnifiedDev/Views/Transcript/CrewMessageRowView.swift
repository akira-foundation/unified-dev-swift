...
/// said something, or Unified Dev is reporting what became of it. `CrewMessage` in the core is the
...
///
/// **A fact gets no rule at all.** `.stopped` and `.failed` are Unified Dev's own voice reporting that a
...
struct CrewMessageRowView: View {
    ...
    // MARK: Unified Dev reporting a fact
...
///
/// Unified Dev's own voice gets no rule at all, and that is the case carrying the meaning: a fact is not
