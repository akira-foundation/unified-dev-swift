...
struct HomeView: View {
    ...
    var body: some View {
        ...
        return VStack(spacing: 0) {
            // The scope segmented control, the order picker and the project menu all moved into
            // the window toolbar; see `WindowToolbar`. What is left here is the one thing that was
            // never a control: the query itself, said back as a removable token now that no field
            // on screen holds it. See `queryHeader`.
            queryHeader
        ...
        // What the toolbar's scope segmented control prints beside each chip's title. Published
        // rather than recomputed in `WindowToolbar`, because the number is the far end of the same
        // pass over every workspace on the machine that builds `listing`, and a toolbar has no
        // business repeating that walk on every rebuild of the window frame.
    ...
    /// What Home is being searched for, said back as a token that can be taken off, over the list
    /// it narrows.
    ///
    /// **This used to be `HomeBar`'s `queryChip`, on a strip above the list.** The strip is gone:
    /// the scope segmented control, the order picker and the project menu all had homes to move to
    /// in the window toolbar, but a removable token naming an active filter is not a control in
    /// the sense the toolbar rules are about, and Apple's own toolbar guidance never mentions one.
    /// It reads as what it is instead, the list's own header, the way Mail heads a search's results
    /// with the terms that produced them.
    ///
    /// **It is not merely decorative.** Home's search field is a panel on Shift+Cmd+F rather than
    /// a field left open on screen, so once a query is narrowing the list there is no box on
    /// screen to backspace it out of. This is the only way left to clear an active search while
    /// its results are still on screen; the empty state's own "Clear search" button only appears
    /// once the query has narrowed the list to nothing.
    @ViewBuilder
