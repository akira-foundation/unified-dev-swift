...
///
/// The row draws no background of its own. `SidebarView.selectionFill` paints an inset neutral
/// grey capsule through `listRowBackground` instead, on every kind of row the pane draws, from one
/// place: AppKit's own source-list fill under Tahoe runs edge to edge and tints with the accent
/// colour while the list has the keyboard, which is neither the shape nor the colour Finder's own
/// sidebar shows.
///
/// Text stays the ordinary label colours rather than an inverted white, because the fill it sits
/// on is never dark or saturated enough to need one. `backgroundProminence` in this pane is
/// therefore always `.standard`; nothing here reaches for `.increased` any more.
