...
/// What Unified Dev offers instead of refusing a folder that is not a git repository.
///
/// Two answers, not one. Conductor's version of this dialog assumes a GitHub repository is what
/// you want, and for Unified Dev that is simply not true: worktrees, agents, diffs and merging back into
...
struct ProjectSetupSheet: View {
    ...
    /// Called with the folder once it is a repository Unified Dev can add, and with nil when the user
        ...
        HStack(spacing: Metrics.spacingWide) {
            ...
            case .failed(let failure):
                if failure.isUsableProject {
                    ...
                    // in it, so it is a project Unified Dev can run, and refusing to add it over a
    ...
    /// is then worked out and, where Unified Dev made it, undone. See `RepositoryStarter.abandon`.
    private func stop() {
        startTask?.cancel()
        startTask = nil
        isStopping = true
