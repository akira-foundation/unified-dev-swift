...
// the path that makes is one Unified Dev is willing to create.
//
...
// apply (Unified Dev's own worktrees, a repository above, the folders macOS owns) are asked here of the
...
extension NewProjectFacts {
    ...
    /// in front of it: `~/dev/code/unifieddev/Tools` is refused for being inside a repository, with the
...
/// Why Unified Dev will not make a project at the path that has been typed.
///
...
public enum NewProjectVerdict: Sendable, Equatable {
    /// Unified Dev makes the folder. `makesLocation` is true when the location above it has to be made
    /// as well, which is the ordinary first run: nobody has a `~/Developer` until something makes
    /// one.
    case create(makesLocation: Bool)
    /// The folder is already there and empty, so Unified Dev takes it as it is.
    ///
    ...
    /// send them to the Finder to delete a folder so that Unified Dev could make the same one back.
    case adopt
...
public extension NewProjectVerdict {
    ...
    /// they make somewhere Unified Dev is willing to work at all, and only then what is already there.
    ...
    /// The line under the location field. It says the whole path and then what Unified Dev will do to
...
// MARK: - The two answers Unified Dev fills in for you

public enum NewProjectPlan {
    /// Where a first project goes when there is nothing to learn it from.
    ///
    /// `~/Developer` rather than a name of Unified Dev's own choosing. It is the one folder name macOS
    ...
    ///
    /// Counted rather than guessed, and counted off the projects Unified Dev already has, because the
    /// honest default for somebody who has been using Unified Dev for a week is the folder they have
    ...
    /// How many of the projects Unified Dev already has live in this folder.
    ///
    ...
    /// Location and name as one absolute path, or nil when the location is not one Unified Dev can place.
