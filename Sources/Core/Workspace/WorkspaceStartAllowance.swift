...
public enum WorkspaceStartAllowance: Sendable, Equatable {
    /// No brake, because the asking is already the brake. The Create sheet, a `unifieddev://` link, the
    ...
    ///
    /// Counted from the database rather than kept in memory, so it survives Unified Dev being reopened
