...
public struct WorkspaceManager: Sendable {
    ...
    /// owner that Unified Dev has just undone something they chose.
    @discardableResult
    func bringProjectBack(_ repoID: RepoID) async -> Bool {
        ...
        // A line in Console because the owner hid this project on purpose and Unified Dev has just
    ...
    /// its own half of the orchestration around it: the create window had all of it, the `unifieddev://`
    ...
    private func cut(
        ...
        // why this can say it and nothing in `Sources/UnifiedDev` can. A workspace with no setup script
    ...
    private func open(
        ...
        //
        // Git allows one worktree per branch, and Unified Dev used to find that out from git: it cut a
        ...
        // mechanisms rather than the only one: every other way in, the bridge and a `unifieddev://`
    ...
    /// The prefix Unified Dev's own interface uses. Everything the app documents, and everything a
    /// script written for it should bind, is `UD_*`.
    public static let environmentPrefix = "UD"

    /// Kept alongside it, and deprecated.
    ///
    /// `CONDUCTOR_*` is not a second interface and nothing here should be read as Unified Dev being
    ...
    ///
    /// Conductor has no equivalent, so this one is Unified Dev's and the deprecated prefix carries it
    ...
    /// `CREATE DATABASE $UD_WORKSPACE_NAME` has one project's worktree writing into the
    /// other's database, and the archive script then drops it out from under them. Every script
    /// author could derive this from `basename $UD_ROOT_PATH`, and the ones who do not think
    ...
    public func environment(for workspace: Workspace, repo: Repo, port: Int) -> [String: String] {
        // The one place `$UD_URL_FILE` is handed out, and therefore the place its folder has to
        // be made to exist: a script told to write to a path whose directory is missing fails with
        // a redirection error, in the middle of setup, over a folder that is Unified Dev's business
    ...
    public func archive(
        ...
        if let archiveLaunch, archiveRuns,
           FileManager.default.fileExists(atPath: workspace.path) {
            ...
            // `$UD_PORT`: the container publishing it, the process still listening on it, the
        ...
        // the repository, so archiving in Unified Dev (Dev) with `delete_branch_on_archive` set deleted a
        ...
        // branch belonging to a worktree Unified Dev has just removed. A worktree Unified Dev never found is a
        // state Unified Dev did not make, and a branch left behind can be deleted by hand later, where a
...
/// can bind `$UD_PORT` without colliding with a sibling workspace.
