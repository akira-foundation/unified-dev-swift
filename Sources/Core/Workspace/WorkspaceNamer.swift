...
/// not an API key, and the token that represents that sign-in lives in `~/.claude.json`. Unified Dev
/// does not read it, does not hold it and does not send it anywhere: the CLI authenticates, the
/// same way it does for every other agent turn this app runs. An HTTP client inside Unified Dev would
/// mean Unified Dev handling a live credential, and would not work at all for the majority of users who
