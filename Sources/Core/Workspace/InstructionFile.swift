...
/// The two things every instruction file Unified Dev sends to an agent has to do: prove it is really
...
public enum InstructionFile {
    ...
    ///
    /// Unified Dev writes this message rather than the user, so there is no caret to put the file at and
    ...
    /// a turn Unified Dev composed and in one somebody typed, so a transcript can draw both the same way.
