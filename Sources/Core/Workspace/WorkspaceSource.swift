...
public enum WorkspaceSource: Sendable, Hashable, Identifiable {
    /// Cut a fresh branch off a named ref. What Unified Dev could always do.
    ...
    /// not exist.
    ///
    /// Only one of the two answers is somewhere Unified Dev can take you. A workspace is selected; a
...
public struct WorkspaceSourceOffering: Sendable, Hashable {
    ...
    /// The "New branch from" section: Unified Dev's original and still ordinary route.
    ...
    /// own checkout or by a worktree Unified Dev did not make. A branch one of Unified Dev's own workspaces
