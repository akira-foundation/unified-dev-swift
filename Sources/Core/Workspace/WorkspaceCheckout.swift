...
public struct ExistingBranch: Sendable, Hashable, Identifiable, Codable {
    ...
    ///
    /// A `BranchHolder` rather than a workspace name, because Unified Dev's own workspaces are not the
...
///
/// The whole point of the feature: Unified Dev could only ever cut a branch, so it was where work is
...
public extension WorkspaceCheckout {
    ...
    /// pull request panel, the checks view and everything else Unified Dev keys by branch. The whole
...
public enum WorkspaceCheckoutPlan {
    ...
    ///
    /// `inUse` is what git says as well as what Unified Dev's database says. See `BranchHolder.byBranch`
