...
///
/// **The bug this exists for.** `ExistingBranch.inUseBy` was filled from Unified Dev's own workspace
/// rows and from nothing else, so a worktree cut by Conductor, by another agent runner, or by
/// hand was invisible to it. The picker drew pull request #362 as a free row, Unified Dev cut a
...
/// which names a folder Unified Dev never mentions anywhere else and reads as a crash rather than as a
/// refusal.
///
...
/// the **name**. A branch held by one of Unified Dev's own workspaces is worth saying by workspace name,
...
public enum BranchHolder: Sendable, Hashable, Codable {
    /// One of Unified Dev's own live workspaces, by name.
    ...
    /// A worktree Unified Dev did not make: Conductor's, another runner's, or one cut by hand.
    ...
    /// intentions, and Unified Dev does not get to pick between them on his behalf.
...
public extension BranchHolder {
    ...
    ///
    /// A workspace name wins over the path of the same worktree. Unified Dev's own worktrees are in the
    ...
    /// inside a function whose whole value is that it is pure. Git prints absolute paths and Unified Dev
