...
// One field, and everything Unified Dev works out from the line typed into it.
//
...
public extension ProjectTarget {
    ...
    /// rule is asked, so "dev/sparkline" is now read as a place that Unified Dev cannot find rather than
...
/// Why Unified Dev will not take the target that was typed.
///
...
public enum ProjectTargetRefusal: Sendable, Equatable {
    ...
    /// is what tells the reader Unified Dev understood the target and will not take it. Which verb is
...
/// What Unified Dev will do about the target that was typed. One of these is named on the button before
...
public enum ProjectTargetVerdict: Sendable, Equatable {
    /// Nothing is there. Unified Dev makes the folder, and the location above it when `makesLocation`.
    case create(makesLocation: Bool)
    /// A folder is there and it is empty, so Unified Dev uses it as it is.
    case adopt
    /// A git repository. Unified Dev registers it and writes nothing.
...
public extension ProjectTargetVerdict {
    ...
    ///
    /// Only where Unified Dev made the repository. A project with an empty first commit is not something
...
public enum ProjectConsequenceTone: Sendable, Equatable {
    /// Nothing typed yet.
    case waiting
    /// Unified Dev will do it, and there is nothing to weigh up.
    case going
    /// Unified Dev will do it, and this is the one to read before pressing: somebody's existing files
...
public extension ProjectConsequence {
    ...
    /// The whole content of this sentence is that Unified Dev wrote none of it. A language-guessed
