...
/// Which folder under the home directory is Unified Dev's workspaces root, decided from what is already
/// on disk.
///
...
public enum WorkspacesRoot {
    ...
    /// The rule, in full:
    ///
    /// 1. If `~/unifieddev/workspaces.noindex` is there, that is the root.
    /// 2. Otherwise, if `~/unifieddev/workspaces` is there, that is the root. This is the whole of what
    ...
    /// asking for the legacy name first would mean an empty `~/unifieddev/workspaces` appearing later,
