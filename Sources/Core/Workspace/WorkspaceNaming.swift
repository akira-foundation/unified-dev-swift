...
/// What a model came back with, once Unified Dev has decided it is usable.
///
...
public enum WorkspaceNaming {
    ...
    ///
    /// Plants, because the app is called Unified Dev and because a workspace called Foxglove sitting in
    ...
    /// overwrite decision compares against the exact string Unified Dev wrote, which is what
    ...
    /// also carries fields Unified Dev must never read back out, so nothing here touches anything but
    /// those two keys.
    ...
    ///
    /// The comparison is against the exact string Unified Dev wrote at creation, not against the shape
...
/// Whether Unified Dev names new workspaces for you.
///
