...
/// backend, a model, an effort and a permission mode; the `unifieddev://` link, the Services menu and
...
public struct WorkspaceStartRequest: Sendable {
    ...
    ///
    /// Nil is the route Unified Dev has always had: a branch named after the prompt, cut from
...
extension WorkspaceManager {
    ...
    /// `unifieddev://` link and the Services menu called the same method with everything after `prompt`
    /// left at its default; and the Shortcuts intent did not call it at all. It built a `unifieddev://`
    ...
    public func start(
        ...
        // the sheet, a `unifieddev://` link, the Services menu, a Shortcut and the bridge alike, so
        ...
        if request.setupPolicy == .run {
            ...
            // wanted to stop whatever holds `$UD_PORT` had nothing to look for while the setup
