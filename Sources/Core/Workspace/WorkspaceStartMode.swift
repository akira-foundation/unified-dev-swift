...
///
/// In `Core` rather than in `CreateWorkspaceView`, because everything below this line is a
...
public enum WorkspaceStartMode: String, CaseIterable, Identifiable, Sendable {
    ...
    ///   to `.standard` would be writing into the owner's own `io.akira.unifieddev` domain, which is
