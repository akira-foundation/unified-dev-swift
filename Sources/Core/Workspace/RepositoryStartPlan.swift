...
// What Unified Dev does about a folder it has been handed, decided before anything is done to it.
//
...
// added anything git recognised, which meant it would register one of Unified Dev's own worktrees or
...
public struct FolderFacts: Sendable, Equatable {
    ...
    ///
    /// Where the location rules are applied, because `~/unifieddev/workspaces/x/src` is inside one of
    /// Unified Dev's worktrees exactly as much as `~/unifieddev/workspaces/x` is, and only the top level
    ...
    /// only from a caller that typed a path, and it matters there because Unified Dev's working
    ...
    /// Where Unified Dev cuts its own worktrees, passed in for the same reason.
...
/// Why Unified Dev will not take a particular folder, whether it was asked to register one or to make
/// one.
///
...
public enum FolderRefusal: Sendable, Equatable {
    /// A path that means nothing on its own. Unified Dev's working directory is not the caller's.
    case notAbsolute(String)
    /// Nothing at that path.
    case nothingThere(String)
    case notADirectory(String)

    /// A worktree Unified Dev cut itself. Registering one would make Unified Dev cut worktrees of a worktree.
...
/// What Unified Dev will do about a folder that was handed to it.
...
public extension FolderVerdict {
    ...
    /// would register one of Unified Dev's own worktrees, or a home directory somebody had run
    /// `git init` in, while `project_add` refused both. They are asked of the resolved top level
    /// rather than of the path handed in, because `~/unifieddev/workspaces/x/src` is inside a worktree
    /// exactly as much as `~/unifieddev/workspaces/x` is and git resolves either.
...
public enum FolderPath {
    ...
    /// string prefix, so `~/unifieddev/workspaces-old` is not read as being inside `~/unifieddev/workspaces`.
    ///
...
///
/// Unified Dev already knows this hazard from two directions. `WorkspaceSafetyReport` singles out a
...
/// in the repository is still copied into every worktree Unified Dev creates, so agents keep working.
///
