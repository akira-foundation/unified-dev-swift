...
/// Whether Unified Dev answers a permission question about one of its own tools without asking anybody.
///
/// ## Why this exists
///
/// Unified Dev's bridge tools reach the agent as MCP tools, and an MCP tool call goes through the CLI's
...
///
/// Unified Dev is on both ends of this question. It wrote the tool, it minted the token the caller is
...
/// weigh that Unified Dev has not already decided, and the ask carries no information a person could
/// act on beyond "an agent would like to use Unified Dev".
///
/// That is emphatically not true of the tools the agent brings with it. `Bash`, `Write` and
/// `Edit` reach outside anything Unified Dev knows about, and nothing here touches them.
///
/// ## What is deliberately not on the list
///
/// Anything that destroys work. `workspace_archive` removes a worktree, and the whole reason Unified Dev
...
/// Unified Dev keeps no copy of what was there before, and both are reachable only from the owner's own
...
/// the owner wrote; a workspace name is a label Unified Dev proposed and the owner accepted, it is one
...
/// and a page he is signed into is his data. Unified Dev cannot tell a dev server's front page from an
...
/// is nothing on the far side of it to restore. Unified Dev answering its own permission question there
/// would be Unified Dev deciding to publish, which is the one decision it has never had.
public enum BridgeToolApproval {
    ...
    /// The tools Unified Dev answers for itself, by their bare names.
    ///
    ...
    public static let selfApproved: Set<String> = [
        ...
        // that Unified Dev has not already decided.
        "agent_start",
        "agent_say",
        "agent_list",
        "agent_stop",
    ...
    /// Whether this ask is Unified Dev answering itself.
    ...
    /// Unified Dev had allowed Unified Dev, and a turn that opens a pane, splits it, renames a tab and lists
