...
/// One browser pane, as much of it as Unified Dev's own toolbar knows.
///
...
public struct BrowserPaneReport: Sendable, Equatable {
    ...
    /// Why this pane is showing Unified Dev's own did-not-load state instead of a page, or nil when it
    /// is showing a page.
    ///
    ...
    /// nothing and `browser_screenshot` with Unified Dev's error card, which a model reads as a blank
    ...
    /// so. The rest is Unified Dev's toolbar read out loud.
    public var json: JSONValue {
        .object([
