...
/// Turning the name a client typed into a project Unified Dev already has.
///
...
/// Unified Dev printed, the path on disk, the name in the sidebar. The id and the path are unique in the
...
public enum BridgeProjectLookup: Sendable {
    ...
    ///
    /// `FolderPath.sameFolder` on both sides, so `~/dev/unifieddev`, `/Users/x/dev/unifieddev` and a path
    /// with a trailing slash are one project rather than three misses, and so is a path reached
    /// through a symlink.
    ///
    /// Public because the `unifieddev://` link resolves a project by path too, and had its own
