...
/// Unified Dev's end of the bridge: one listening socket for the whole app, one connection per running
...
public final class BridgeServer: Sendable {
    ...
    /// The same, for the conversation that belongs to Unified Dev rather than to a workspace.
    ///
    ...
    /// names a shim binary and a socket that will answer it with "Unified Dev does not recognise this
    ...
    /// the same fingerprint as the socket beside it, so Unified Dev and Unified Dev (Dev) cannot read each
    ...
    /// running Unified Dev is told about the version rather than about an unknown token, which is what
