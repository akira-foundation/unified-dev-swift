...
/// What an agent asked Unified Dev to start, once the arguments have been read and checked.
///
...
public struct AgentWorkspaceOrder: Sendable, Hashable {
    ...
    /// Nil lets Unified Dev name it the way it names any other workspace.
    ///
...
extension AgentWorkspaceOrder {
    ...
    /// amount of plumbing inside Unified Dev will make it appear.
    ///
...
/// `workspace_start`: an agent asking Unified Dev for another workspace in the same project.
///
/// **Not `workspace_spawn`, and nothing here says "subagent".** A workspace an agent asked for is
/// an ordinary Unified Dev workspace with an ordinary agent in it, which is what `WorkspaceOrigin` says
/// in as many words and what the seam is called. The vocabulary is load-bearing: a name that
/// implied a lesser kind of workspace would be describing something Unified Dev does not have.
///
...
/// Setup and the opening turn run afterwards, under Unified Dev's own runners, while the caller carries
...
/// nothing else says which, and it may only name one Unified Dev already has.
///
