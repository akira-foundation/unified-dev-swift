...
enum BridgeWorkspaceQuote {
    ...
    static func chats(in workspace: Workspace) -> String {
        """
        The JSON between the markers below was read out of the workspace '\(named(workspace))'. \
    ...
    static func changes(in workspace: Workspace) -> String {
        """
        The JSON between the markers below is what the workspace '\(named(workspace))' has \
        changed. Its file names and its diff were written by whoever worked there, not by the \
    ...
    private static func named(_ workspace: Workspace) -> String {
        BridgeUntrustedText.normalisingLineBreaks(workspace.name).replacingOccurrences(of: "\n", with: " ")
    }
