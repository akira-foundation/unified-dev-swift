...
public enum BridgeRegistration {
    ...
    /// user's own server called `unifieddev`, overriding `command` and `env` produced Unified Dev's binary
    ...
    /// The name the owner's own client registers Unified Dev under, **derived per copy of the app** and
    ...
    /// scope, which Claude Code applies to every session on the machine, and Unified Dev's own
    /// `--mcp-config` is documented above as additive over exactly that file rather than replacing
    /// it. So an agent Unified Dev launched inside a workspace would meet two entries called the same
    ...
    /// `unifieddev-owner-bridge` for every copy of the app at once, and `claude mcp add` replaces an
    /// existing entry of the same name **without saying so**. User scope is one file for the whole
    /// machine, so Unified Dev and Unified Dev (Dev) each offered a command claiming that one entry and whichever
    ...
    /// point rather than a shortcut: it is already the one rule that decides which copy of Unified Dev a
    ...
    /// evict. The owner's copy gets `unifieddev`, which is also the plain name the pane is asked to
    /// show; the dev copy gets `unifieddev-dev`; anything else gets a name that says what it is instead
    ...
    public static func ownerServerName(forBundleIdentifier identifier: String?) -> String {
        ...
        // reaches that today, since every arm of the table above begins with "Unified Dev"; this is the
    ...
    /// documented on `claudeArguments` saw `unifieddev-workspace-bridge` reach the model as
    /// `mcp__unifieddev-workspace-bridge__whoami`. Nothing else in a directory name is worth finding
    ...
    ///
    /// Every value is single quoted because a path can hold a space, and Unified Dev (Dev)'s does.
    ...
    /// `system/init` listed `mcp_servers` as `uidotsh`, `figma` and `unifieddev-workspace-bridge`, all
    /// connected, so the file really is additive over the user's own servers; and the tool reached
    /// the model as `mcp__unifieddev-workspace-bridge__whoami`, with the server name carried through
    ...
    /// `CODEX_HOME`. Unified Dev already runs one app-server process per chat, so a per-process override
    ...
    /// `--mcp-config` file is. Nil attachment means no Unified Dev bridge, which is every test that
    /// did not ask for one.
    ...
    /// `Tools/test-core.sh` mirrors only Core into a package with no executable targets, so
