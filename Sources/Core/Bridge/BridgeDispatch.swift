...
/// The MCP server, on Unified Dev's side of the socket.
///
...
public struct BridgeDispatch: Sendable {
    ...
    /// What Unified Dev calls itself in `serverInfo`. The version is the socket protocol's, not the
    ...
    /// point of the handshake that already ran on this socket is that version skew is Unified Dev's
