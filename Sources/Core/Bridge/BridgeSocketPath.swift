...
/// the same `TmuxSessions.fingerprint`, so Unified Dev and Unified Dev (Dev) can never land on one socket. That
...
/// `~/Library/Application Support/Unified Dev/`, is already around 45 characters before the filename and
