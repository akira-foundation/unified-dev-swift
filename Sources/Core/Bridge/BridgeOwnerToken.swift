...
/// session token. A session token is minted for a process Unified Dev is about to launch, the argv
...
///
/// The standalone token identifies no process Unified Dev starts. It identifies a client configuration
/// that Unified Dev hands to a person once and then never sees again: it is pasted into the user's own
...
/// readable by anything running as the user. Keeping Unified Dev's copy in the Keychain would protect
...
///
/// Third, a Keychain item is access-controlled by signing identity, and Unified Dev is re-signed ad hoc
...
public struct BridgeOwnerToken: Sendable {
    /// Where it lives: beside the database, so the separation between Unified Dev and Unified Dev (Dev) that
    ...
    /// nothing to lose by minting again, and the alternative is a Unified Dev that cannot be coupled to
    ...
    ///
    /// This is revocation. The old token is not written anywhere else by Unified Dev, so overwriting the
