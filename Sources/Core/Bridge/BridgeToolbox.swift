...
public struct BridgeToolbox: Sendable {
    ...
    /// `acceptEdits`, calling `whoami` produced an ask for `mcp__unifieddev-workspace-bridge__whoami`
    ...
    /// `workspace_report` unattended therefore needs an answer of its own, either a grant Unified Dev
    /// makes for its own tools or an allow rule in the child's settings. `BridgeToolApproval` is
    /// where that was answered, and its head says which tools Unified Dev answers for and why the rest
