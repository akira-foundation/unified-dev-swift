...
/// The contract between `bridge`, the stdio shim Unified Dev ships inside its own bundle, and the
...
/// share between Unified Dev and Unified Dev (Dev), which are a documented permanent pair. So the registered
...
/// bundle underneath a running Unified Dev and the CLI launches whatever binary is at the path its
...
public enum BridgeProtocol {
    ...
    ///
    /// The skew to design for is a NEW shim meeting an OLDER running Unified Dev: Sparkle swaps the
    ...
    /// sentence cannot serve both. A session token is minted for a process Unified Dev is about to
    /// launch and is held in memory only, so a quit really does retire it and reopening Unified Dev
    ...
    /// the owner regenerates it. Sending that caller away to quit and reopen Unified Dev is sending it
