...
/// The whole of `bridge`, the stdio shim Unified Dev ships in its own bundle.
///
/// It speaks no MCP. After a hello it copies lines from stdin to the socket and from the socket to
/// stdout, and Unified Dev answers `initialize`, `tools/list` and `tools/call` at the other end. That
...
///
/// It lives in Core rather than in the executable target because `Tools/test-core.sh` mirrors
/// only Core and its tests into a throwaway package: an executable target is invisible to the
...
public enum BridgeShim {
    ...
    /// exited 1" says nothing that "Unified Dev is not running" does not say better.
    public enum Exit {
    ...
    public static func run(
        ...
        do {
            ...
            // The ordinary case, and it deserves a sentence rather than an errno: Unified Dev has quit
            // and somebody is running the CLI by hand from a config file Unified Dev left behind.
        ...
        // the handler below closed the socket, which is an ordinary shutdown, or Unified Dev went away
        // while the CLI was still talking to it. The shim could not tell them apart and reported
        // both as success, so a `tools/call` sent to a Unified Dev that quit mid-answer left the CLI
    ...
    /// stdin to the socket, off the same readability handler the rest of Unified Dev uses for a pipe.
    ///
