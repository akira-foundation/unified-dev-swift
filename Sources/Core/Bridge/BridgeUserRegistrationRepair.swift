...
/// Re-points the owner's durable user scope entry when Unified Dev's bundle has moved, and refuses to
...
///
///     "command": "/Users/freek/Applications/UnifiedDev.app/Contents/MacOS/bridge"
///
/// The owner then dragged Unified Dev from `~/Applications` to `/Applications`. Every agent started from
...
/// per-session registration Unified Dev writes for a workspace agent was correct throughout. The fault
/// is only in the one entry that is written once and then never re-derived.
///
/// ## What counts as ours, and stale
///
/// Re-pointing an entry Unified Dev did not write, or one deliberately aimed at another copy of Unified Dev,
...
///   derived from `Store.databaseDirectoryName`, so the release copy only ever looks at `unifieddev`,
///   the dev copy at `unifieddev-dev` and the subagents copy at `unifieddev-subagents`. The three identities
///   in `Tools/guard.sh` therefore cannot fight over one entry: they are not looking at the same
///   one.
/// - Its `UD_BRIDGE_SOCKET` and `UD_BRIDGE_TOKEN` are **exactly what this instance would
///   hand out today**. The socket is derived from the database path through
///   `TmuxSessions.fingerprint`, which is the one rule that says which copy of Unified Dev a process is,
...
/// - Its `command` names a file called `bridge`. Belt and braces over the pair above, and it
...
///   environment, so a Unified Dev (Dev) shim driving the release copy's socket is a working arrangement,
...
///
/// ## Why this may write a file `BridgeUserRegistration` says Unified Dev does not write
///
/// That file's rule is that Unified Dev does not compose a person's configuration for them, which is why
/// the feature is a command to copy rather than a write, and it still stands. Correcting a path
/// inside an entry Unified Dev itself minted, which it can prove it minted, and which currently names
/// nothing at all, is repairing Unified Dev's own record rather than editing somebody's setup.
public enum BridgeUserRegistrationRepair {
    ...
    public enum Reason: String, Sendable, Hashable {
        ...
        case alreadyCorrect
        /// Pointing somewhere else, at a `bridge` that is still there. Not stale, so not
    ...
    ///     could not be read.
    ///   - name: the server name this copy of Unified Dev registers under. Always
    ///     `BridgeRegistration.ownerServerName` in the app; passed in so a test can name one.
    ///   - attachment: what this copy of Unified Dev would hand out today.
