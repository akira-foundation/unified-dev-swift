...
/// Whether the owner's own Claude Code has already been told about this copy of Unified Dev.
///
...
/// and two values Unified Dev itself minted.
///
/// **It never writes.** Unified Dev does not compose a person's own configuration file, which is the
...
/// from under an entry this app minted, and which says at length why that is repairing Unified Dev's own
/// record rather than editing somebody's setup.
public enum BridgeUserRegistration {
    /// What the user scope says about this copy of Unified Dev.
    ///
    ...
    public enum State: String, Sendable, Hashable {
        /// An entry under this Unified Dev's name, pointing at this Unified Dev's shim, with this Unified Dev's
        /// socket and token in it.
        case registered
        /// No entry, or one that names this Unified Dev and points somewhere else. Those two are one
        /// case on purpose: an entry left behind by a copy of Unified Dev that has been replaced is a
    ...
    ///   - attachment: what this copy of Unified Dev would hand out today, or nil when it has no bridge
    ...
    /// Whether an entry is this Unified Dev's rather than something else wearing its name.
    ///
