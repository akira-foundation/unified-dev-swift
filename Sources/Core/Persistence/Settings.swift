...
public struct RepoSettings: Sendable, Hashable {
    ...
    /// What this project adds to the turn Unified Dev sends when someone presses Merge, and the one it
    /// sends for Fix merge conflicts. Unified Dev's own words are not here and never were: for merging
    /// they are in the message it composes, and for conflicts they are the file
    /// `ConflictInstructions` writes. These are the project's, they go after Unified Dev's, they win
    ...
    /// `.unifieddev/merge-instructions.md` beat these.
    public var mergeInstructions: String?
    public var conflictInstructions: String?
    /// What a browser pane opens on, before expansion, or nil for the port Unified Dev allocated.
    ///
    /// Stated with the variables a script is handed, so one line covers every workspace:
    /// `http://localhost:$UD_PORT/admin`. A workspace whose setup script wrote an address of
    ...
    /// Set by a machine-wide file (`~/.conductor/settings.toml`, `~/.unifieddev/settings.toml`).
    ///
...
public enum SettingsLoader {
    ...
    ///
    /// Relative to the repository rather than to the settings file, so `.unifieddev/setup.sh` reads
    /// the same whichever of the four files names it, and so a path stays meaningful when a value
    /// is promoted from `.conductor` to `.unifieddev`.
    ...
    static func apply(
        ...
        // states. That matters now that Unified Dev writes to `.unifieddev` and never to `.conductor`, so
        ...
        // missing here: a repository configured in Conductor copied nothing in Unified Dev, silently,
...
public struct AppDefaults: Sendable, Hashable {
    ...
    /// What a copy of Unified Dev with no row for `defaults.backend` runs on, which is every copy that
    ...
    /// and Unified Dev exists to run several at once.
    ///
    /// This is a fallback, not an override. The moment `defaults.permissionMode` holds anything
    /// at all, the Settings, Models picker wins, which is why a copy of Unified Dev whose Models tab
    ...
    public static func load(from store: Store) async -> AppDefaults {
        ...
        // so the pair a copy of Unified Dev was already carrying stays one coherent choice.
