...
///
/// The settings a repository has are already stated in files: `<repo>/.unifieddev/settings.toml` and
/// its `.local` sibling, plus the `.conductor` pair of the same shape, which is where a
/// repository already configured for Conductor keeps them. Unified Dev reads all four. The obvious way
/// to make the screen editable is to keep overrides in Unified Dev's own database and layer them on
...
///   A value only Unified Dev's database knows is a value nothing else can honour.
/// * "Edit it in the app, then look at the file" has to show the change. That is only true if the
///   app edits the file.
///
/// So the app edits the files, and the only question left is which one. The answer is: **Unified Dev's
...
///
/// The one value that is not edited where it was found is one that came from `.conductor`. Unified Dev
/// reads that folder so a repository already set up for Conductor works here with nothing to
/// configure; it does not follow that Unified Dev should maintain another product's file. So the edit
/// goes to `.unifieddev`, which outranks it, and the old line is left exactly as the team committed
...
public enum SettingsWriter {
    ...
    ///
    /// Always one of Unified Dev's own two files. A value read out of `.conductor/settings.toml` is not
    /// edited where it was found: Unified Dev reads that folder so an existing Conductor repository
    /// works with nothing to configure, which is a courtesy on the way in and not a claim on the
    /// folder. Writing there would make Unified Dev quietly maintain another product's configuration.
    ///
    /// A value only a machine-wide file states is not edited in place either, for the older
    /// reason: this screen is about one repository, and rewriting `~/.unifieddev/settings.toml` from it
    /// would change every other repository on the machine.
    ///
    /// The tier is kept. A value that came from a `.local` file is written to Unified Dev's `.local`
    ...
    /// `.unifieddev/settings.toml` would leave the old value still winning, and the edit would look
    /// like it did nothing.
    ...
    /// Unified Dev's file at the same tier as `path`: shared for shared, `.local` for `.local`.
    ...
    /// Unified Dev's folder in a repository, made ready the first time something is written into it.
    ///
    /// `.unifieddev` is meant to be committed, and that is the whole point of it. `settings.toml` and
    ...
    /// than in Unified Dev's database.
    ///
    ...
    ///
    /// The distinction matters because `.unifieddev` is routinely brought into existence by something
    /// that has no business writing the team's ignore rules: `WorktreeScratch` makes
    /// `.unifieddev/attachments` and `.unifieddev/scratch` inside it, and those folders must add nothing to
    ...
    ///
    /// A program does; one command does not. `npm run dev` inside `.unifieddev/run-dev.sh`, under a
    ...
    /// `.unifieddev/.gitignore` keeps `*.local.sh` out of `git` for the same reason it keeps
    ...
    ///    it is not quietly repointed at Unified Dev's own name for it, and a file that has gone missing
    ...
    public static func scriptFile(
        ...
        // not Unified Dev's call to make from a cleared text box.
    ...
    ///
    /// Only ever true because Unified Dev writes to `.unifieddev` and reads `.conductor` as well. Deleting a
    /// line from `.unifieddev/settings.toml` would leave the `.conductor` one showing through, so
    ...
    static func apply(
        ...
        // keep it in a file of its own already has `.unifieddev/merge-instructions.md`, which wins.
    ...
    private static func writeRunScripts(
        ...
        for script in scripts {
            ...
            // Only when it says something the id does not. `name` is Unified Dev's own key, and writing
