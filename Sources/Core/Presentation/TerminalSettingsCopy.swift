...
/// test: `Tests/CoreTests` depends on the core alone. Three states for where a size comes from
/// and two for whether terminals can outlive a quit, and getting one of them wrong means telling
/// somebody Unified Dev is following a configuration it has stopped following.
public enum TerminalSettingsCopy {
    /// Where the terminal text size is coming from, and nothing else.
    ///
    /// "13 pt" on its own cannot say whether Unified Dev is following the user's own terminal or has
