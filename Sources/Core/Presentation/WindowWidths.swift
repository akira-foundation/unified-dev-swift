...
///
/// It was one line in `UnifiedDevApp`: the sidebar's maximum, plus the detail half's minimum, plus a
...
/// clipped Create Pull Request button that `UnifiedDevApp`'s old comment is about. `presenting` is the
