...
/// opened straight onto four probes, which meant the first thing a new Unified Dev ever said to anybody
...
/// owns the sequence rather than this enum.** Unified Dev can be driven from the owner's own terminal by
/// running one `claude mcp add` command, and until it was offered here it lived in a settings pane
/// nobody browses. It is shown only when there is something to offer: this copy of Unified Dev has a
...
public enum OnboardingStep: String, Sendable, Hashable, CaseIterable, Identifiable, Codable {
    /// The mark, the name and one line saying what Unified Dev is. No information to act on.
    case greeting
    ...
    /// Keep Awake that Unified Dev cannot do on its own. Optional twice over: only a Mac with a lid is
    ...
    case keepAwake
    /// The one command that couples the owner's own Claude Code to this Unified Dev. Optional, and
    /// omitted entirely when there is nothing to offer. See `OnboardingFlow.steps`.
    case commandLine
    /// The prompt anybody can send back to the people who build Unified Dev, which is the same sheet
    ...
    /// a form. This one wants nothing. It says Unified Dev is free, gives an address, and leaves. A
    ...
    /// using Unified Dev" here and does exactly that.
    ...
    /// not do is read as a job: Unified Dev works perfectly without it.
    ///
