...
/// What a project has to say about one of Unified Dev's own turns, over and above what Unified Dev says.
///
/// Unified Dev's own words used to be a file as well. Pressing Merge wrote `MergeInstructions` into
/// `.unifieddev/scratch/merge-instructions.md` and attached it straight back, so the turn could say
...
/// turn arrives here already carrying a file of Unified Dev's, and what this type adds goes after it and
/// outranks it.
///
/// **Two places a project can say it, and the file wins.**
///
/// - `.unifieddev/merge-instructions.md`, or `.unifieddev/conflict-instructions.md`, in the worktree. This
...
public enum ProjectInstructions {
    /// One turn Unified Dev composes itself and that a project may add to.
    ...
    /// `.unifieddev/merge-instructions.md`, or one somebody emptied and left behind, has said nothing,
    /// and a turn that names it would send the agent to read nothing. Judging by the words in it
    /// stops there: a file with a heading and nothing else is somebody starting, and Unified Dev is in
    ...
    /// moment of answering, which is the contract every path Unified Dev writes into a turn holds: a
    ...
    /// The whole turn: what the prompt rendered, Unified Dev's own steps for this subject, and the
    ...
    /// Unified Dev's own steps, which no override can edit away.
    ///
    ...
    ///
    /// Resolving a conflict has steps of Unified Dev's own too, and they are not nil because there are
    ...
    /// file Unified Dev names in a turn goes and how a person typing one writes a path. See
