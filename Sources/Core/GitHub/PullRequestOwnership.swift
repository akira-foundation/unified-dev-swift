...
/// reused constantly, by people and by Unified Dev itself, which derives one from the task and from a
...
///   branch's own git config, so a review workspace can say which one is its without Unified Dev
...
public extension Git {
    ...
    /// Reading it back costs one `git config` and needs no column of Unified Dev's own: the association
...
public extension GitHub {
    ...
    /// both sides, the worktree moved to `main`, and every route Unified Dev had ran out at once. `gh pr
    ...
    /// deleted is the state Unified Dev's own merge leaves behind, and the only state route 3 helps in.
