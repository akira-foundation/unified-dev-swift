...
///
/// Every id in Unified Dev used to be a bare `String`, so `store.update(workspaceID: session.id)`
...
///
/// The raw value is `task_id` off `system/task_started`, and it is the CLI's, not Unified Dev's: it is
