...
/// asking Unified Dev's own database. The database only knows about worktrees Unified Dev cut. Git knows
...
public enum WorktreeListing {
    ...
    /// which is the shape of mistake that ends in Unified Dev telling somebody the wrong folder to go
    /// and close.
    ///
