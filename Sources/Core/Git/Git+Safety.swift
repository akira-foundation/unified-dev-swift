...
/// files and `git worktree remove` deletes them without a word. Unified Dev copies `.env*` into every
...
/// - `.unifieddev` is ours. The app creates it to hold prompt attachments, so listing it as work at
///   risk is Unified Dev warning the user about Unified Dev.
...
public struct WorkspaceSafetyReport: Sendable, Hashable {
    ...
    /// without a word, so these were invisible. Unified Dev copies `.env*` into every worktree, which
...
extension Git {
    ...
    /// An ignored file that matches the main checkout byte for byte is not a loss: Unified Dev copied it
