...
///
/// Model ids and permission modes reach Unified Dev verbatim from a settings file or from a session's
/// own init line, so the ones Unified Dev ships a label for are a list that is always behind. Everything
...
/// branch in it and the test target cannot reach `Sources/UnifiedDev`. It got there by being wrong:
...
public enum ModelLabel {
    ...
    ///
    /// The vendor prefix goes: inside Unified Dev every model is a Claude model, so the word carries no
