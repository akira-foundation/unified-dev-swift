...
public extension AgentKind {
    func interactiveCommand(
        ...
        let command = TerminalLaunchScript.shellCommand(
            ...
            arguments: [
                ...
                "UD_CLI_STATUS_FILE=\(Self.interactiveStatusURL(sessionID: sessionID).path)",
                executableName
            ] + arguments
        )
    ...
    static func interactiveStatusURL(
        ...
        return base.appendingPathComponent("unifieddev-cli-status", isDirectory: true)
    ...
    static var interactiveHookCommand: String {
        #"umask 077; if [ -n "$UD_CLI_STATUS_FILE" ]; then mkdir -p "$(dirname "$UD_CLI_STATUS_FILE")" 2>/dev/null && ud_status_tmp=$(mktemp "$UD_CLI_STATUS_FILE.XXXXXX") && { cat > "$ud_status_tmp" && mv -f "$ud_status_tmp" "$UD_CLI_STATUS_FILE"; } 2>/dev/null; fi; exit 0"#
    }
