...
///
/// Keyed by **repository**, not by workspace. A Unified Dev workspace is a git worktree, so anything
...
///
/// Nothing here is a settings file. Unified Dev writes none, and answers matching asks itself.
public struct PermissionGrant: Identifiable, Sendable, Hashable, Codable {
    ...
    ///
    /// Here rather than in each runner because it is Unified Dev's own bookkeeping rather than either
...
/// whitespace, no resolving of paths. If any of those were done here, Unified Dev would be deciding
/// that two rules mean the same thing, and Unified Dev is not the component that knows that: a rule is
/// a string in a syntax the CLI owns and evolves.
///
/// The consequence is deliberate and worth stating. Unified Dev under-matches rather than over-matches.
/// A rule Unified Dev cannot recognise means one more question, which costs a click. The opposite
/// mistake means a command running because Unified Dev decided it resembled something the user once
...
/// without Unified Dev knowing what `*` means.
public enum PermissionGrantIndex {
    ...
    /// The sentence the transcript prints when Unified Dev answered instead of the user.
    ///
...
/// for as long as it takes, and there is no timer on the other end, so a Unified Dev that forgot the
...
/// later Unified Dev has to be readable by this one: an unrecognised word means "decided, somehow",
