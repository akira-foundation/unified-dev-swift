...
/// Supervises one `claude` process for one Unified Dev session.
///
...
public actor AgentRunner {
    ...
    /// global on Homebrew's PATH and a current native install in `~/.local/bin`, and Unified Dev
    ...
    ///
    /// An effort Unified Dev does not know about is passed through rather than filtered. Effort is an
    ...
    func ingest(_ event: AgentEvent) async {
        ...
        // A result for a turn Unified Dev never asked for closes nothing, and it is dropped here rather
        ...
        case .initialized(let info):
            ...
            // Unified Dev sent.** The CLI runs a background task's completion notification as a turn of
            ...
            // `turnStarted` is `unchanged` from `running`, which is what a turn Unified Dev sent is by
    ...
    private func handle(_ ask: PermissionAsk) async {
        // Unified Dev's own bridge tools answer themselves, and they do it before anything is stored or
        ...
        // settled question saying Unified Dev allowed Unified Dev is a second row about a call the transcript
    ...
    public func answer(requestID: String, decision: PermissionDecision) async {
        ...
        // Granting is Unified Dev's own bookkeeping and happens after the CLI has been unblocked, so a
...
public extension PermissionMode {
    /// What `--permission-mode` expects. Unified Dev's names happen to line up with the CLI's today,
