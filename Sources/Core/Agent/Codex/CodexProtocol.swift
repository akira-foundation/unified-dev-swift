...
///     is finished. Unified Dev's transcript types as the model does, and `exec` cannot do that.
...
///     There is no wire on which a question could arrive, which is the same failure Unified Dev already
...
/// also what Conductor drives. So Unified Dev takes app-server, and `exec` is never used.
///
...
///     decoder that requires `"jsonrpc": "2.0"` drops every line. Unified Dev still sends it, because a
...
/// structured detail there that no Unified Dev release has to understand in advance.
...
/// Builds the lines Unified Dev writes to the server's stdin.
///
