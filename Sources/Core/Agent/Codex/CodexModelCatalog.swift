...
/// cannot use it.
///
/// **The efforts belong to the model, not to Unified Dev.** Claude Code takes the same five levels for
...
/// each. Not one line of Unified Dev changed to offer any of it, which is the whole case for fetching:
...
/// real control rather than an unreachable field, and Unified Dev sends neither, so every Codex turn
/// runs at standard speed. It is left out on purpose and not for want of a place to put it: it
/// spends the user's allowance faster, and Unified Dev's own "Fast mode" switch is already a different
...
/// keeping: it is what a picker can put under the name instead of Unified Dev inventing one.
...
public actor CodexModelCatalog {
    ...
    /// `cwd` is an empty folder Unified Dev owns rather than the home directory: listing models opens no
