...
///
/// The eighteen types are the ones `ThreadItem` names in the generated schema. The ten Unified Dev will
...
public struct CodexUserMessage: Sendable, Hashable {
    ...
    /// Unified Dev's own attachment drafts already produce, so nothing about them has to change.
...
/// and a Unified Dev row needs them: one app-server connection can carry several threads.
...
public struct CodexTokenUsage: Sendable, Hashable {
    ...
    /// drive either the context gauge or the per-turn usage Unified Dev stores.
    ...
    /// Poured into the shape the rest of Unified Dev already stores and draws.
    ///
...
/// One thing the server said, in the vocabulary Unified Dev's transcript can eventually draw.
///
...
public enum CodexEvent: Sendable {
    ...
    /// addressed to, and it is the server's own numbering, not Unified Dev's.
...
/// One of the five questions the server can ask, held open until Unified Dev answers it.
///
...
/// server to remember rather than asking Unified Dev to.
...
/// Claude Code there is no rule for Unified Dev to store and replay.
