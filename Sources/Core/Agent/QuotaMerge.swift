...
/// Folds a fresh report about an allowance into what Unified Dev already knew about it.
///
...
///
/// Unified Dev read that notification as the whole truth. Everything on Codex's `RateLimitWindow` is
...
public enum QuotaMerge {
    ...
    /// Unified Dev know now", but writing every untouched row back would restamp its `observedAt` and
...
///
/// **A number with no age on it is a claim about now.** Unified Dev's figures arrive when a provider is
