...
/// carried a `cachedRepoID` property, a `repoID()` under the same comment about Ask Unified Dev having
...
/// the same paragraph about doing Unified Dev's bookkeeping after the agent has been unblocked. That is
...
/// none of this is protocol. It is Unified Dev deciding what it already approved, in Unified Dev's own tables,
...
final class SessionGrants: Sendable {
    ...
    func repoID() async -> RepoID? {
        ...
        // Nil for a chat with no worktree, which is Ask Unified Dev: there is no project behind it, so
