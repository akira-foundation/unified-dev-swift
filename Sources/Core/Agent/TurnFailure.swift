...
/// "API Error: Connection lost mid-response. The response above may be incomplete." Unified Dev read
...
///
/// # Unified Dev's sentence over the CLI's, with the CLI's kept
///
...
///
/// What is not defensible is passing it through as though Unified Dev had written it, because then the
/// one time a person most needs the app to be plain, the app is quoting. So a terminal reason
/// Unified Dev recognises gets Unified Dev's own lead sentence in Unified Dev's own register, saying what happened
/// and whether anything is asked of the reader; the CLI's words follow, marked as the CLI's. A
/// reason Unified Dev does not recognise gets no lead at all, and the CLI's words stand alone rather
/// than being wrapped in a sentence that pretends to a diagnosis nobody made.
public struct TurnFailure: Sendable, Hashable {
    /// Unified Dev's own account, or nothing when Unified Dev has no reading of this ending.
    ...
    /// is the honest answer to an ending Unified Dev has never seen.
