...
/// A `result` line that closes a turn nobody in Unified Dev asked for.
///
...
///
/// Unified Dev read that `result` because every `result` used to end whatever turn was open. It wrote
...
public enum StrayResult {
    /// Whether this line closes a turn Unified Dev never started.
    ///
    ...
    ///
    /// The emptiness is the net under that reading. No turn Unified Dev sent comes back with no API
