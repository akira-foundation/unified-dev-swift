...
///
/// **Standing in `~` is not free, because the child is not Unified Dev.** macOS attributes what a
/// subprocess touches to the responsible application, so anything the CLI reads there is asked for
/// in Unified Dev's name, and the user is asked about it by a system prompt naming Unified Dev. Both agent CLIs
...
/// Unified Dev did, before it had been shown a single project, was name the user's home directory as the
...
/// looking for configuration walks `~/Library/Application Support/Unified Dev`, `~/Library` and then `~`
