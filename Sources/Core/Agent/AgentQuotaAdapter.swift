...
/// the type to `AgentQuotaAdapters.all`. Nothing else in Unified Dev changes: the store is keyed by
...
/// of the CLIs Unified Dev detects are exactly that, and they contribute nothing rather than
...
/// Every adapter Unified Dev has, and the one call the app makes.
...
/// second window, so what Unified Dev knows is whatever has been mentioned since the last reset, which
...
///
/// **There is no monthly window.** `five_hour` and `seven_day` are what this protocol emits. Unified Dev
...
public enum ClaudeCodeUsageAdapter: AgentQuotaAdapter {
    ...
    /// is the CLI's own rather than `NumberFormatter`'s fraction digits, so Unified Dev and `/usage`
