...
public struct CrewMessage: Sendable, Equatable, Codable {
    ...
    public enum Event: String, Sendable, Codable {
        ...
        case said
        /// Unified Dev reporting that an agent's turn ended. Not an agent talking.
        case stopped
        /// Unified Dev reporting that an agent did not finish.
        case failed
    }

    /// Who is speaking, which is what the rule down the left is coloured by. A fact is Unified Dev's own
    ...
    /// Unified Dev telling an orchestrator that one of its subagents has stopped, carrying what that
