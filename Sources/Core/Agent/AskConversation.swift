...
/// The chat that belongs to Unified Dev rather than to a workspace: what it is called, where it runs,
...
public enum AskConversation {
    ...
    /// Without host context, Ask Unified Dev picked an installed Conductor skill to create workspaces.
    ...
    /// gives: `Store.databaseDirectoryName` is the one rule that decides which copy of Unified Dev a
    /// process is, so Unified Dev and Unified Dev (Dev) get their own without a second rule to keep in step.
    ///
    ...
    /// under `.unifieddev/attachments` here, exactly as it would in a worktree, and the agent can then
    ...
    /// worktree is Unified Dev's word for a thing the reader has not met yet, and an empty pane that
