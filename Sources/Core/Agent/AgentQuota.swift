...
///   on this protocol, and there is no way to ask for the others. So the windows Unified Dev knows about
...
/// Claude's names are the only thing it gives and a name Unified Dev has never seen carries no length.
public struct QuotaWindow: Sendable, Hashable, Codable, Identifiable {
    ...
    /// must be whatever the payload said and never a Unified Dev invention.
    ...
    /// name is not one Unified Dev can read.
...
public struct AgentQuota: Sendable, Hashable, Identifiable {
    ...
    /// When Unified Dev last heard this. Kept so a late arriving report cannot overwrite a fresher one,
    ...
    /// was refilled while nobody was watching and the only honest reading is that Unified Dev does not
...
/// Everything Unified Dev knows about every provider's allowances, arranged for reading.
///
