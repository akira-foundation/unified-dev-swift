...
final class AgentLifecycleSpool: Sendable {
    ...
    init(capacity: Int = 64 * 1_024 * 1_024) {
        ...
        url = FileManager.default.temporaryDirectory.appendingPathComponent("unifieddev-lifecycle-\(UUID().uuidString)")
    }
    ...
    private enum Failure: Error, LocalizedError {
        ...
        var errorDescription: String? {
            switch self {
            case .capacity: "The live conversation buffer is full. Unified Dev stopped the agent to keep its turn history consistent."
            case .unavailable: "Unified Dev could not preserve the live conversation updates. The agent was stopped."
            }
        }
    }
}
