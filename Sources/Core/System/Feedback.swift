...
/// The two things somebody can deliberately send from Unified Dev: a feedback submission, and a prompt
/// for the agent that builds Unified Dev.
///
...
/// they carry what they were given: a message, optionally an excerpt of Unified Dev's own log, and
...
/// from nothing, and joinable against nothing but another thing this copy of Unified Dev sent. See
...
public enum Feedback {
    ...
    /// The endpoint's own ceiling on the log field. Unified Dev's excerpt is capped at a third of it by
    ...
    /// How much picture Unified Dev will put in one request.
    ///
    ...
    public enum InstallSource: String, Sendable, Equatable, CaseIterable, Codable {
        ...
        /// the value that is always true of a build that is not a release. Unified Dev's build scripts
    ...
    /// a bug report needs, because a translated Unified Dev behaves like an Intel Unified Dev.
    ///
    ...
    /// Everything Unified Dev is allowed to say about where it is running, and nothing else.
    ///
    ...
    /// Unified Dev could not establish is better missing than guessed.
    ///
    ...
    public struct Environment: Sendable, Equatable, Encodable {
        ...
        /// Which coding agent Unified Dev runs here, as `InstallPing.agentName(installed:)` answers it.
        ...
        /// The language and region Unified Dev is being read in, as a language tag: `nl-BE`. Built from
        ...
        /// The block as it goes out, in a fixed order, with everything Unified Dev could not establish
        /// left out.
        ///
    ...
    /// The shape an address has to have before Unified Dev will send it.
    ///
    ...
    /// the at sign is allowed to be case sensitive and it is not Unified Dev's business to decide it is
    /// not.
    ...
    /// attempted yet.
    ///
    /// Nothing before the first attempt, because a half-typed `freek@akira-io.` is not wrong, it is
    ...
    ///
    /// **The name is Unified Dev's, not the user's.** The endpoint does not store a client filename, and
    ...
    public struct Image: Sendable, Equatable {
        ...
        /// `attachment.png`. Unified Dev's own name for it, and always the extension that goes with what
    ...
    /// A prompt somebody wants run against Unified Dev itself, and the name to credit it to.
    ...
    /// a server being printed into Unified Dev's own interface: a reply that is not JSON, or that
