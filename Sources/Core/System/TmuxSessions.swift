...
/// alive across a quit.
///
/// One session per Unified Dev *pane*, never per window: `SplitLayout` already owns the geometry, so
...
public enum TmuxSessions {
    ...
    /// is not loaded into shells Unified Dev drew, so their prefix key and status line stay theirs.
    ///
    /// The socket is per database, so an instance pointed at `UD_DB_PATH` cannot adopt or sweep
    ...
    /// Sessions that nothing in Unified Dev can ever reach again.
    ///
    ...
    /// edit outside Unified Dev left behind.
    ...
    /// killed. Nothing running is allowed to depend on Unified Dev having remembered it.
    ...
    /// of a tmux-backed pane is a child of the server rather than of Unified Dev, so this is the only
    ...
    public static func parsePanePIDs(_ output: String) -> [String: Int32] {
        ...
        for line in output.split(separator: "\n") {
            ...
            // The first pane of a session wins. Unified Dev's own sessions hold exactly one, and a
...
/// One resolved tmux binary on one private socket, and every command line Unified Dev sends it.
///
...
public struct TmuxCommand: Sendable, Equatable {
    ...
    /// Re-applies the configuration to a server that was already running when Unified Dev launched,
