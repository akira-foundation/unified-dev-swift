...
public enum InstallPing {
    ...
    /// When this copy of Unified Dev first ran. See `firstSeenAt(in:now:)`.
    ...
    /// placeholder, because unlike the appcast this is a host Akira Foundation owns and a path that exists.
    ...
    ///
    /// The same kind of development affordance as `UD_DB_PATH`: it exists so that working on
    ...
    ///
    /// A local build with `UD_PING_URL` set does send, to whatever that names. That is the
    /// combination that makes this feature developable, and it is also what makes it impossible
    /// for a development build to reach `unified-dev.akira-io.com` by accident: no override, no ping.
    ...
    /// The value `Tools/build.sh` writes into `BuildChannel` for a build that was given a version.
    ...
    /// thing is sent. Unified Dev has no onboarding and asks nothing on first launch, which is the right
    ...
    /// day they install Unified Dev and turns this off is never counted at all, not even once.
    ///
    ...
    ///
    /// Under a day on purpose. With exactly 24 hours, somebody who opens Unified Dev at nine, works for
    ...
    /// it can ever be joined against is another ping from the same copy of Unified Dev, which is exactly
    ...
    /// `~/Library/Preferences/io.akira.unifieddev.plist`. It does not survive somebody deleting Unified Dev's
    ...
    ///
    /// So the number means "copies of Unified Dev whose preferences have survived", not "people". Two
    ...
    /// What is sent when Unified Dev cannot run a turn at all on this machine, because the CLI it drives
    ...
    /// Which agents this copy of Unified Dev can actually run turns with, given what is installed.
    ///
    ...
    /// `PATH` is not Unified Dev using Cursor, so only backends `AgentKind.canRunWorkspaces` admits to
    ...
    public struct Payload: Sendable, Equatable, Encodable {
        ...
        /// Which coding agent Unified Dev runs here. See `agentName(installed:)`.
