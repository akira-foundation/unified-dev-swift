...
/// Which icon a browser tab wears, and what Unified Dev will accept as one.
///
...
/// and it is Unified Dev opening a connection to whichever host a page happens to be on for a file
/// nobody asked for. An icon a page does not declare is one Unified Dev does not go looking for.
///
/// **What comes back is never the page's own bytes.** The page rasterises its icon into a canvas
/// Unified Dev sized and hands back a PNG of that, so no ICO parser, no SVG renderer and no image of a
...
public enum BrowserFavicon {
    ...
    private struct Ranked {
        ...
        /// Nil for a declaration Unified Dev will not ask for: a `rel` naming no icon, an href that will
    ...
    /// capped before any decoding, so a page cannot make Unified Dev allocate its way through a
