...
/// When this copy of Unified Dev was built, for the one place that has a use for it: the line under the
...
/// `BuildDate` into the assembled bundle beside `BuildChannel`, which is the same place
...
public enum BuildTimestamp {
    /// The Info.plist key `Tools/build.sh` writes. In the `Unified Dev` prefix the other two build facts
