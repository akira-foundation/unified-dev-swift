...
/// The last few minutes of what Unified Dev told its own log, made fit to send with a feedback report.
///
/// Unified Dev logs almost nothing, and what it does log is the one thing a bug report can never
...
/// before they decide.
///
/// **What comes out.** A Unified Dev log line can name a workspace, a project, a branch, a path, and
...
/// it happened to.
///
/// **What can never be in it.** The log is Unified Dev's own writing, so nothing here ever opens a file:
...
public enum AppLogExcerpt {
    ...
    /// What the View link shows when there is nothing to show, which is the ordinary case: Unified Dev
    ...
    ///
    /// Handed in rather than discovered, because Core cannot know what the user called their
    ...
    /// `<workspace>` says plainly that Unified Dev removed a name, where a blanked run of x's reads as
    /// corruption.
    ...
    /// because Unified Dev's log has one purpose and every line in it is worth the same. Newlines are
    ...
    /// One line with everything that is not Unified Dev's business taken out of it.
    ///
    ...
    /// characters that could be base64 is not a word anybody typed, and no sentence Unified Dev writes
