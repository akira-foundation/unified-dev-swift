...
///
/// `URL(string:)` refuses a `unifieddev://` link whose prompt has not been percent encoded: a space or
/// a colon anywhere in the payload returns nil, so `Unified Dev --open-url "unifieddev://prompt=fix: the
...
public enum OpenURLArgument {
    ...
    private static func repaired(_ argument: String) -> URL? {
        ...
        // The same shape `DeepLink.values(from:)` reads back: pairs on `&`, one `=` each.
