...
///
/// Safari's tab reads "Akira" when the page at `https://akira-io.com/` says so, and Unified Dev's read
...
/// changed**. Not "Loading": that is a third string, and a tab that reads Akira, then Loading,
/// then GitHub in one second flickers, which is exactly what a label you navigate by must not do.
///
/// The host is what makes it honest. Following a link inside `akira-io.com` keeps "Akira" up while
...
public enum BrowserTabTitle {
    ...
    /// The host a tab shows when the page has no title of its own: `akira-io.com`, `localhost:3000`.
    ///
