...
/// Whether this copy of Unified Dev is allowed to update itself, when it may look, and what it says when
...
public enum SoftwareUpdate {
    ...
    ///
    /// A day. Unified Dev is a tool people leave open for days at a time, so an interval measured in
    ...
    ///
    /// `Tools/master.sh` installs into `~/Applications/UnifiedDev.app` under the real bundle id, and stamps
    ...
    /// The heading of the alert shown when an install would restart Unified Dev over running agents.
