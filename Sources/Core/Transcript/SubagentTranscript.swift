...
/// need for the fallback to Unified Dev's own nested rows, and this is the honest source.
///
...
///
/// Unified Dev does not own the file, which is why nothing here throws on a shape it does not know: a
...
public struct SubagentTranscript: Sendable, Equatable {
    ...
    /// The same reading, taken off Unified Dev's own stored rows rather than off the CLI's file.
    ///
    ...
    /// read and the pane had nothing to say. Unified Dev is not blind for that period: every line the
    ...
    /// task, and this is what Unified Dev happened to see while it was being written.
    ///
    ...
    ///
    /// One line means one row throughout Unified Dev, and `AgentEvent` reads the first block of a
    ...
    /// do to a line Unified Dev does not own.
...
///
/// A file Unified Dev does not own, in a directory Unified Dev does not own, written by another process that
