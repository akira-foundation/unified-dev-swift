...
public enum FilePathGuess {
    ...
    ///
    /// Accepts `HarbourMap.php`, `Sources/UnifiedDev/Views/Palette.swift`, `/Users/x/notes.md`.
    ...
    /// - directories, because a directory has no extension to recognise, and `Sources/UnifiedDev` and
    ///   `Sources/UnifiedDev.swift` are told apart by nothing else
    ...
    ///
    /// Unified Dev opens a file by handing the review a path relative to the worktree, and an agent
    ...
    ///
    /// A relative path is taken at face value. Every agent Unified Dev runs is spawned with the worktree
