...
/// asserting on, and this file's own note said they would move into `Core` once the sending
...
public enum PromptAttachments {
    ...
    ///
    /// Inside, rather than in Unified Dev's application support directory, because the agent has to be
    /// able to read it. Claude Code will not read a path outside its working directory without
    /// asking, Unified Dev has nothing that can answer that question, and a turn that stalls on an
    /// unanswerable permission prompt is worse than no attachments at all. Inside the worktree the
    /// read is ordinary.
    ///
    /// Under `.unifieddev`, which is already Unified Dev's corner of a repository, and made invisible to git
