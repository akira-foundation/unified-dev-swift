...
public struct Subagent: Sendable, Hashable, Identifiable {
    ...
    /// When `task_started` arrived, by Unified Dev's own clock, so a row can count its own seconds when
    ...
    /// and Unified Dev's own otherwise, because a fan-out that produces no ticks at all is a real thing
...
/// and running ones are kept, and only the process going away ends one Unified Dev was not told about.
///
...
/// already and Unified Dev does not own that file; and a table would mean a migration, a delete pass and
