...
/// `.unifieddev/attachments/9JVKW4/shot.png` and fix the spacing" reaches it as one thought instead of
...
/// self-delimiting, so `.unifieddev/attachments/9JVKW4/Pasted 2026-08-20 at 22.29.20.png` survives its
...
///
/// **What counts as one.** Either the path is Unified Dev's own copy, under `.unifieddev/attachments`, which
...
public struct AttachmentDraft: Equatable, Sendable {
    ...
    /// The folder every attachment Unified Dev copies into a worktree is written to. A path under it is
    ...
    /// Unified Dev's own folder in somebody's checkout. A file inside it is a file this app either wrote
    ...
    ///
    /// Two ways to be one. Either it is a file in Unified Dev's own folder, which needs no telling: a
    /// copy under `.unifieddev/attachments`, the pull request instructions under `.unifieddev/scratch`, or
    /// the project's own `.unifieddev/pr-instructions.md`. That is what lets a sent turn draw the same
    ...
    /// path the agent cannot read only teaches it that Unified Dev lies about paths.
