...
/// Instructions Unified Dev put into a turn it composed, in the words the agent was actually handed.
///
/// Unified Dev writes four kinds of instruction into the turns it sends on the owner's behalf, and
...
///
/// Three runs, and the last two are one thing said two ways. A file Unified Dev named in the sentence is
/// a path the reader can point at; a block of instructions Unified Dev put in the message itself is
...
/// was that he could hover one and not the other. Two drawings of "here is what Unified Dev added to
...
///
/// The parsing is a pure function of the stored text and of the constants Unified Dev appends, so it
...
public enum SentTurn {
    ...
    public enum Segment: Equatable, Sendable {
        ...
        /// decides what counts.
        case file(String)
        /// A block Unified Dev appended whole.
    ...
    /// The title a file Unified Dev named in its own turn is drawn under, or nil for any other file.
    ///
    ...
    ///
    /// Matched on the exact paths Unified Dev writes into a turn, and nothing looser, so a file somebody
    ...
    /// Every block Unified Dev ever appends whole, as the constant it is appended from.
    ///
    ...
    ///
    /// `retiredDefaults` is in here for the reason it exists at all: a turn sent by an older Unified Dev
    ...
    private static func firstBlock(in text: String, from start: String.Index) -> Found? {
        ...
        // A project's own words are not a constant, so what is recognised is the sentence Unified Dev
    ...
    ///
    /// Unified Dev joins the parts of a turn with a blank line and never with anything else, so a match
    /// that starts mid sentence is somebody quoting rather than Unified Dev appending. It is the same
