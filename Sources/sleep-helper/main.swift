...
/// The one thing Unified Dev cannot do for itself: turn the system's sleep switch off while a Keep Awake
...
/// sudoers rule granting every admin account passwordless `pmset`. Unified Dev is not sandboxed, so it
/// can use the door Apple opened instead: a `LaunchDaemon` registered with `SMAppService`, approved
/// once, which changes nothing outside Unified Dev and leaves nothing behind when Unified Dev is deleted.
///
/// It does exactly two things, and it is deliberately this small: it flips the switch, and it
/// watches the process that asked. If Unified Dev dies with sleep disabled, the watchdog puts it back.
...
let machServiceName = "io.akira.unifieddev.sleep"

/// Only Unified Dev may ask. Checked by the system rather than by us: `setConnectionCodeSigningRequirement`
...
and (identifier "io.akira.unifieddev" or identifier "io.akira.unifieddev.dev")
"""
