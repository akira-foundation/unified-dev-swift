...
/// with its backend written in front of it. Nothing in Unified Dev composes that string; it arrives
...
struct ModelIdentifierTests {
    ...
    /// A colon whose left side is not a backend Unified Dev has. Some other tool's namespace is not
    /// Unified Dev's to unpick, and the open-set rule stands: keep it, show it, let it be changed.
