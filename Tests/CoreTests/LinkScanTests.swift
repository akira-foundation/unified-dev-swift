...
struct LinkScanTests {
    ...
    @Test("a local development host is a link", arguments: [
        "http://unifieddev.test",
        "https://unifieddev.test/one/two?three=four#five",
    ...
    @Test("a file path is not a link", arguments: [
        ...
        "./build.sh",
        "~/Library/Application Support/Unified Dev/unifieddev.sqlite",
    ...
    func gitRemote() {
        #expect(written("git@github.com:akira-io/unifieddev.git").isEmpty)
        #expect(written("origin  git@github.com:akira-io/unifieddev.git (fetch)").isEmpty)
    }
    ...
    func mailAddress() {
        #expect(written("write to freek@akira-io.com about it").isEmpty)
        #expect(written("mailto:freek@akira-io.com").isEmpty)
    }

    @Test("a bare host with no scheme is not a link")
    func bareHost() {
        #expect(written("read it on akira-io.com").isEmpty)
    ...
    @Test("a scheme Unified Dev does not open is not a link")
    func otherSchemes() {
        #expect(written("file:///Applications/UnifiedDev.app").isEmpty)
