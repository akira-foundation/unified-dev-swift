...
struct PromptRegistryTests {
    ...
    func createPullRequestDefaultRendersFully() throws {
        ...
        let context = PullRequestPromptContext(
            ...
            changes: "- Sources/Core/PromptTemplate.swift (added, +90/-0)"
        )
...
struct PullRequestInstructionsTests {
    @Test("Unified Dev's own copy is written into the shielded scratch folder, not next to the user's work")
