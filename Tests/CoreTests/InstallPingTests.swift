...
struct InstallPingTests {
    ...
    func requestShape() throws {
        ...
        #expect(request.value(forHTTPHeaderField: "User-Agent") == "Unified Dev/0.4.0")
    ...
    func overrideIsHonoured() {
        let local = ["UD_PING_URL": "http://127.0.0.1:8787/api/install-reports"]

        #expect(endpoint(channel: nil, environment: local)?.absoluteString == local["UD_PING_URL"])
        #expect(endpoint(channel: "release", environment: local)?.absoluteString == local["UD_PING_URL"])
    }

    @Test("refuses an endpoint that is not http")
    func overrideMustBeHTTP() {
        #expect(endpoint(environment: ["UD_PING_URL": "file:///tmp/ping.json"]) == nil)
        #expect(endpoint(environment: ["UD_PING_URL": "not a url at all"]) == nil)
        #expect(endpoint(channel: "release", environment: ["UD_PING_URL": "   "])?.absoluteString
    ...
    func defaultEndpointIsTheAgreedOne() throws {
        ...
        #expect(url.host == "unified-dev.akira-io.com")
