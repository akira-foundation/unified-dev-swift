...
struct FeedbackTests {
    ...
    func reportKeys() throws {
        let json = try FeedbackFixture.object(FeedbackFixture.report(logs: "10:00:00  archive  nope"))
    ...
    func reportWithoutLogs() throws {
        let json = try FeedbackFixture.object(FeedbackFixture.report())

        #expect(Set(json.keys) == ["message", "token", "environment"])
    }

    @Test("a token that was not generated by Unified Dev is left out rather than sent")
    func tokenIsChecked() throws {
        let json = try FeedbackFixture.object(FeedbackFixture.report(token: "/Users/someone/.claude.json"))
    ...
    func promptKeys() throws {
        let json = try FeedbackFixture.object(
            Feedback.PromptSubmission(
                prompt: "make the sidebar narrower", name: "Freek", email: nil, token: FeedbackFixture.token,
    ...
    func promptWithoutName() throws {
        let json = try FeedbackFixture.object(
            Feedback.PromptSubmission(
                prompt: "make it narrower", name: "   ", email: nil, token: nil, environment: FeedbackFixture.environment()
            )
        )
    ...
    func environmentKeys() throws {
        let json = try FeedbackFixture.object(FeedbackFixture.environment())
    ...
    func nothingPersonalSurvives() throws {
        let hostile = FeedbackFixture.environment(
    ...
    func logsAreCarriedVerbatim() throws {
        ...
        let json = try FeedbackFixture.object(FeedbackFixture.report(logs: excerpt))
    ...
    private func image(_ data: Data = FeedbackFixture.pngBytes, declaring type: String = "image/png") -> Feedback.Image {
    ...
    func multipartShape() throws {
        let body = try Feedback.body(for: FeedbackFixture.report(images: [image()]), boundary: "TESTBOUNDARY")
        let written = FeedbackFixture.text(of: body)
    ...
    func multipartAndJSONAgree() throws {
        let written = FeedbackFixture.text(of: try Feedback.body(for: FeedbackFixture.report(images: [image()]), boundary: "B"))
        let json = try FeedbackFixture.object(FeedbackFixture.environment())
    ...
    func imageCountCap() {
        ...
        #expect(FeedbackFixture.report(images: images).images.count == Feedback.maxImages)
    }
    ...
    func messageCap() {
        let long = FeedbackFixture.report(message: String(repeating: "a", count: Feedback.maxMessageCharacters + 500))
    ...
    func endpointOverride() {
        let found = Feedback.endpoint(
            .report, environment: ["UD_FEEDBACK_URL": "http://127.0.0.1:8787/api/feedback-submissions"]
        )
    ...
    func endpointOverrideIsChecked() {
        #expect(Feedback.endpoint(.report, environment: ["UD_FEEDBACK_URL": "file:///tmp/out.json"]) == nil)
        #expect(
            Feedback.endpoint(.report, environment: ["UD_FEEDBACK_URL": "   "])?.absoluteString
    ...
    func requestShape() throws {
        let body = try Feedback.body(for: FeedbackFixture.report())
    ...
    func multipartRequestHeader() throws {
        let body = try Feedback.body(for: FeedbackFixture.report(images: [image()]), boundary: "XYZ")
        let request = Feedback.request(to: URL(string: "https://example.test/x")!, body: body, appVersion: "0.4.0")
