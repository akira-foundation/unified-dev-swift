...
/// `WorkspaceManager.archive` is the only thing in Unified Dev that deletes a git worktree and can
...
struct WorkspaceArchiveTests {
    ...
    /// The archive script used to be handed `UD_PORT=0`, whatever block the workspace was
    ...
    func ignoredFilesAreInvisibleToTheReport() async throws {
        ...
        // Unified Dev itself copies `.env*` into every new worktree, so this is the file most likely to
        ...
        // deletes them silently. Unified Dev copies `.env*` into every worktree, so this was the
