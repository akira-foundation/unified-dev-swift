...
struct CrewToolTests {
    ...
    /// `agent_list` reads rows and reaches nothing else, so a Unified Dev with no app behind it serves
    ...
    /// **Unified Dev sweeps nothing**, so a finished subagent leaves the owner's sidebar when the
