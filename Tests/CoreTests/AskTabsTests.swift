...
struct AskTabsTests {
    ...
    @Test func closingWithAStaleSelectionFallsBackToTheNeighbour() {
        let sessions = (0..<3).map { _ in AskConversation.newSession() }
        let stale = AskConversation.newSession().id
        #expect(AskTabs.selectionAfterClosing(sessions[1].id, selected: stale, sessions: sessions) == sessions[0].id)
        #expect(AskTabs.selectionAfterClosing(sessions[0].id, selected: nil, sessions: sessions) == sessions[1].id)
    }
