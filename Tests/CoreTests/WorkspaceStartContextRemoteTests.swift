...
struct WorkspaceStartContextRemoteTests {
    ...
    @Test("a fetched branch list shows a branch published after the last fetch, and drops a deleted one")
    func branchListCatchesUpWithTheRemote() async throws {
        ...
        let before = await WorkspaceBranchListing.read(repoPath: work.path)

        let fetched = await WorkspaceStartContext.fetchBranchList(repoPath: work.path)
        let after = await WorkspaceBranchListing.read(repoPath: work.path)

        #expect(!before.primaryRemoteBranches.contains("colleague/idea"))
        #expect(before.primaryRemoteBranches.contains("gone/soon"))
        #expect(fetched)
        #expect(Set(after.primaryRemoteBranches) == ["main", "colleague/idea"])
