...
/// What a typed identifier has to keep doing, now that every id in Unified Dev is one.
///
...
struct IdentifierTests {
    /// The conformance the whole change rests on, and it is not Unified Dev's.
    ///
    ...
    /// copied field for field: it is the only value Unified Dev stores as JSON with an id inside it, and
