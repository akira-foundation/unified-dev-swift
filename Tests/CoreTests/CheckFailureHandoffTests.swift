...
struct CheckFailureHandoffTests {
    ...
    func readsJob() {
        let target = CheckFailureHandoff.logTarget(
            detailsURL: "https://github.com/akira-io/unifieddev/actions/runs/42/job/99"
        )
    ...
    func readsRerun() {
        let target = CheckFailureHandoff.logTarget(
            detailsURL: "https://github.com/akira-io/unifieddev/actions/runs/42/attempts/2/job/99"
        )
        #expect(target == CheckFailureHandoff.LogTarget(runID: "42", jobID: "99"))
    }
    ...
    func readsRun() {
        let target = CheckFailureHandoff.logTarget(
            detailsURL: "https://github.com/akira-io/unifieddev/actions/runs/42"
        )
    ...
    func writesTheSentence() {
        ...
        let sentence = CheckFailureHandoff.sentence(
            ...
            state: .failed,
            detailsURL: "https://github.com/akira-io/unifieddev/actions/runs/42/job/99",
            logPath: ".unifieddev/attachments/aB3dEf/Run tests.log",
            excerpt: excerpt
        )
        #expect(sentence.contains("\"Run tests\""))
        #expect(sentence.contains("in CI"))
        #expect(sentence.contains("failed."))
        #expect(sentence.contains(AttachmentDraft.token(for: ".unifieddev/attachments/aB3dEf/Run tests.log")))
    ...
    func theSentenceIsAValidDraft() {
        let path = ".unifieddev/attachments/aB3dEf/Run tests.log"
