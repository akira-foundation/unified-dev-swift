...
struct NoticeLifetimeTests {
    ...
    func scales() {
        ...
        let long = "Unified Dev named this workspace Describe fade-in animation feel. Its branch is still "
    ...
    func dismissal() {
        #expect(Notice(message: "Unified Dev named this workspace Foxglove.").lifetime != nil)
        #expect(Notice(message: "It moved.", dismissal: .untilDismissed).lifetime == nil)
    }
}
...
struct NoticeTextTests {
    ...
    func split() {
        let text = NoticeText("Unified Dev named this workspace Foxglove. Its branch is still `add-a-toggle`.")
        #expect(text.fact.map(\.text).joined() == "Unified Dev named this workspace Foxglove.")
    ...
    func plain() {
        let message = "Unified Dev named this workspace Foxglove. Its branch is still `add-a-toggle`, because it has 2 commits on it."
