...
/// Merging is the one destructive, off-machine thing Unified Dev offers, and it is now done by an agent
...
///
/// They were a file until they were not: written into `.unifieddev/scratch/merge-instructions.md` on
...
struct MergeInstructionsTests {
    ...
    /// would do both, and its local half fails from a worktree, which is the whole reason Unified Dev
    ...
    /// Unified Dev is an app built on worktrees, and the copy on this machine is the thing a reader is
...
/// no in words the reader can act on. A button that refused first would be Unified Dev guessing an
