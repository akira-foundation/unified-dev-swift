...
struct WorkspaceSayToolTests {
    ...
    func delivers() async throws {
        ...
        #expect(row.source == WorkspaceMessageEnd(
            workspaceID: f.fixer.id, workspace: "fix-the-bug", project: "apex",
    ...
    func recordReadsBack() async throws {
        ...
        let record = try #require(WorkspaceSayRecord(
            toolName: "mcp__unifieddev-workspace-bridge__workspace_say", input: input, resultText: result.text
        ))
        ...
        #expect(WorkspaceSayRecord(toolName: "mcp__unifieddev-workspace-bridge__agent_say", input: input, resultText: result.text) == nil)
        #expect(WorkspaceSayRecord(toolName: "mcp__unifieddev-workspace-bridge__workspace_say", input: input, resultText: "Refused.") == nil)
    }
    ...
    func routeRoundTrips() throws {
        let message = WorkspaceMessage(
            source: WorkspaceMessageEnd(workspaceID: WorkspaceID("w"), workspace: "fix", project: "apex", chat: "Chat"),
            target: WorkspaceMessageEnd(workspaceID: WorkspaceID("t"), workspace: "release"),
            text: "Hi."
        ).crewMessage
    ...
    func childReach() async throws {
        ...
        func workspace(_ name: String, origin: WorkspaceOrigin = .user) async throws -> Workspace {
            try await store.upsert(Workspace(
                repoID: repo.id, name: name, branch: "apex/\(name)",
