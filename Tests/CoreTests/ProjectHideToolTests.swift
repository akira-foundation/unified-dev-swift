...
struct ProjectHideToolTests {
    ...
    private func seed(_ store: Store) async throws -> (Repo, Repo) {
        let ember = try await store.upsert(Repo(name: "ember", path: "/tmp/ember", sortOrder: 0))
        let unifieddev = try await store.upsert(Repo(name: "unifieddev", path: "/tmp/unifieddev", sortOrder: 1))
    ...
    func hides() async throws {
        ...
        let (ember, _) = try await seed(store)

        let result = await ProjectHideTool()
            .call(request("project_hide", "ember"), as: .owner, store: store)
    ...
    func hidingTwice() async throws {
        ...
        _ = await tool.call(request("project_hide", "ember"), as: .owner, store: store)
        let again = await tool.call(request("project_hide", "ember"), as: .owner, store: store)
    ...
    func unhides() async throws {
        ...
        _ = await ProjectHideTool()
            .call(request("project_hide", "/tmp/ember"), as: .owner, store: store)

        let back = await ProjectUnhideTool()
            .call(request("project_unhide", "ember"), as: .owner, store: store)

        #expect(!back.isError)
        #expect(back.text.contains("\"state\" : \"showing\""))
        #expect(try await store.repo(id: ember.id)?.hidden == false)
    ...
    func hidingEverything() async throws {
        ...
        _ = await tool.call(request("project_hide", "ember"), as: .owner, store: store)
        let last = await tool.call(request("project_hide", "unifieddev"), as: .owner, store: store)
    ...
    func ambiguousProject() async throws {
        ...
        _ = try await store.upsert(Repo(name: "ember", path: "/tmp/one/ember"))
        _ = try await store.upsert(Repo(name: "ember", path: "/tmp/two/ember"))

        let result = await ProjectHideTool()
            .call(request("project_hide", "ember"), as: .owner, store: store)

        #expect(result.isError)
        #expect(result.text.contains("/tmp/one/ember"))
        #expect(result.text.contains("/tmp/two/ember"))
    ...
    func nothingRegistered() async throws {
        ...
        #expect(result.isError)
        #expect(result.text.contains("Unified Dev has no projects"))
