...
struct InteractiveAgentTests {
    ...
    func literalPrompt() async throws {
        let root = FileManager.default.temporaryDirectory.appendingPathComponent("unifieddev-interactive-\(UUID())")
    ...
    func writesHook() async throws {
        let root = FileManager.default.temporaryDirectory.appendingPathComponent("unifieddev-hook-\(UUID())")
        defer { try? FileManager.default.removeItem(at: root) }
        let destination = AgentKind.interactiveStatusURL(sessionID: SessionID("../unsafe"), base: root)
        #expect(destination.deletingLastPathComponent().deletingLastPathComponent().lastPathComponent == "unifieddev-cli-status")
        ...
        let result = try await Shell.run("/bin/sh", ["-c", AgentKind.interactiveHookCommand],
                                         env: ["UD_CLI_STATUS_FILE": destination.path], stdin: payload)
        #expect(result.ok)
        ...
        let failed = try await Shell.run("/bin/sh", ["-c", AgentKind.interactiveHookCommand],
                                         env: ["UD_CLI_STATUS_FILE": "/dev/null/impossible"])
        #expect(failed.ok)
    ...
    func isolatedStatusFiles() {
        ...
        let production = AgentKind.interactiveStatusURL(sessionID: id, namespace: "io.akira.unifieddev")
        let development = AgentKind.interactiveStatusURL(sessionID: id, namespace: "io.akira.unifieddev.dev")
