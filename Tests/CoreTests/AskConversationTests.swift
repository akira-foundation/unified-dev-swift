...
struct AskConversationTests {
    /// The permission decision, not the tidiness one. Unified Dev's default mode is `acceptEdits`, and a
    ...
    /// Beside the database rather than at a fixed path, so the separation between Unified Dev and Unified Dev
    ...
    /// chat cannot do, in Unified Dev's own vocabulary ("this chat has no worktree"). It says what to ask
