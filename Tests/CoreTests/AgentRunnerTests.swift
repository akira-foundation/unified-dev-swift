...
struct AgentRunnerArgvTests {
    ...
    /// Unified Dev has no name for, which is why `ComposerOption.adding` exists. Passing it through is
    ...
    func fastModeOn() {
        ...
        // starts, so the only thing Unified Dev may ever send here is a literal.
    ...
    /// The picker that started this: Claude Code gained output styles, and Unified Dev offers them in
    ...
    /// A style is named by a file somebody else wrote, so the name is not Unified Dev's to trust. Built
...
struct AgentRunnerPersistenceTests {
    ...
    /// an `init` and closed by a real `result`, without Unified Dev sending it anything. Nothing moved
...
struct AgentRunnerProcessTests {
    ...
    func runsATurn() async throws {
        ...
        // 25 event rows plus the user turn Unified Dev wrote itself.
...
struct AgentRunnerPermissionTests {
    ...
    /// has no project to store a rule against. Ask Unified Dev's card does not offer the button, and
