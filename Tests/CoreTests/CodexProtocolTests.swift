...
@Suite struct CodexEventTests {
    @Test func readsAWholeTurnOffTheRecording() throws {
        ...
        // The methods Unified Dev does not lift out are still seen, with their names, rather than lost.
...
@Suite struct CodexModelTests {
    ...
    /// set from the five Unified Dev hardcodes today.
    ...
    /// Unified Dev had to be edited to offer it. A hardcoded list would have missed all of it.
    @Test func readsAGenerationOfModelsNothingHereHadHeardOf() throws {
        ...
        // The name the picker draws is the server's, so a model Unified Dev has never heard of is still
        ...
        // Its own default rather than Unified Dev's `high`, which is why the model carries one.
