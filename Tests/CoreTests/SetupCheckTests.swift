...
private func report(
    ...
    codex: SetupOutcome = .ready(detail: "freek@akira-io.com"),
...
struct SetupFixTests {
    ...
    @Test("only the interactive commands claim Unified Dev can run them")
    func interactivity() {
...
struct SetupToolTests {
    ...
    @Test("every agent Unified Dev can drive has a row, so a new backend cannot be forgotten")
...
struct SetupAccountLineTests {
    ...
    func prefersEmail() {
        let status = AgentStatus(
            ...
            version: "2.1.234",
            details: [
                AgentDetail(label: "Organization", value: "Akira"),
                AgentDetail(label: "Email", value: "freek@akira-io.com"),
            ]
        )
        #expect(SetupProbe.accountLine(status) == "freek@akira-io.com")
    }
    ...
    func skipsUnknown() {
        ...
        #expect(SetupProbe.accountLine(status) == "Akira")
    }
