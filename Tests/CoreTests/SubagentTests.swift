...
@Suite struct SubagentRowTests {
    ...
    /// are always the nested rows Unified Dev stored, and because the file is named on the line that
    ...
    /// and never moved. Unified Dev's own clock is the floor under it.
...
@Suite struct SubagentTranscriptTests {
    ...
    @Test func aLineThatWillNotParseIsSkippedRatherThanEndingTheRead() {
        // Unified Dev does not own this file, so a shape it does not know degrades to fewer rows.
