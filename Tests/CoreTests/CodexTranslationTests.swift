...
@Suite struct CodexPermissionTests {
    ...
    /// The narrowest rule there is. Unified Dev will not invent a pattern, because Codex offers no
    ...
    /// the rest of this session, Unified Dev keeps the durable record itself, and no file belonging to
    /// the user is written.
...
@Suite struct CodexPatchCountingTests {
    /// A Codex patch states its own numbers. Unified Dev's Claude Code counting estimates them from the
