...
struct SlashCommandTests {
    ...
    @Test("shared and Codex user skills appear in the slash menu", arguments: [
    ...
    func skillsThroughASymlink() throws {
        ...
        #expect(tree.names().contains("ember"))
    }
    ...
    func commandsCarryTheirFile() throws {
        ...
        let ember = try #require(found.first { $0.name == "ember" })
    ...
    func documentationStripsFrontmatter() throws {
        ...
        let path = try tree.write(
            ...
            Run `ember errors` to list them.
            """
        )

        let found = try #require(SlashCommandIndex.documentation(of: path, lines: 24, columns: 160))
        #expect(found.lines == ["# Ember", "", "Run `ember errors` to list them."])
    ...
    @Test("the built in list only holds things a Unified Dev turn can actually carry out")
