...
extension TurnCheckpointTests {
    ...
    func rejectedRestoreDoesNotPrepareARewind(_ obstruction: String) async throws {
        ...
        let refs = try await Shell.check("git", ["for-each-ref", "--format=%(refname)", "refs/unifieddev/checkpoints"], cwd: repo.path)
        ...
        #expect(try await Shell.check("git", ["for-each-ref", "--format=%(refname)", "refs/unifieddev/checkpoints"], cwd: repo.path).stdout == refs.stdout)
