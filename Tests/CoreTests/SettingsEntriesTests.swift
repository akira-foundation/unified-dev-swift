...
struct SettingsEntriesTests {
    private func makeRepo(_ files: [String: String] = [:]) throws -> String {
        let root = TestScratch.unique("unifieddev-entries")
    ...
    func malformedFile() throws {
        ...
        #expect(!settings.sources.contains(path(repo, ".unifieddev/settings.toml")))
        let found = issues(settings, in: repo)
        #expect(found.count == 1)
        #expect(found.first?.path == path(repo, ".unifieddev/settings.toml"))
    ...
    func missingRunScriptFile() throws {
        let repo = try makeRepo([".unifieddev/settings.toml": "[scripts.run.dev]\nfile = \".unifieddev/gone.sh\"\n"])
        let settings = SettingsLoader.load(repo: repo)
        ...
        #expect(issues(settings, in: repo).map(\.message) == [
            "Run script \u{201C}dev\u{201D} cannot run: .unifieddev/gone.sh does not exist.",
        ])
    }
    ...
    func validQuickPrompts() throws {
        ...
        #expect(settings.quickPrompts == [
            ProjectQuickPrompt(
                ...
                symbol: "terminal", source: path(repo, ".unifieddev/settings.toml")
            ),
            ProjectQuickPrompt(
                name: "Write the changelog", text: "Write a changelog entry.",
                opensNewChat: true, source: path(repo, ".unifieddev/settings.toml")
            ),
        ])
    ...
    func unknownSymbolFallsBack() throws {
        let repo = try makeRepo([
            ".unifieddev/settings.toml": "[[quick_prompts]]\nname = \"A\"\nprompt = \"B\"\nsymbol = \"not.a.symbol\"\n",
        ])
        let settings = SettingsLoader.load(repo: repo)
        #expect(settings.quickPrompts.first?.symbol == QuickPrompt.defaultSymbol)
        #expect(issues(settings, in: repo).map(\.message) == [
            "Quick prompt \u{201C}A\u{201D} shows the default symbol: \u{201C}not.a.symbol\u{201D} is not one Unified Dev offers.",
        ])
    ...
    func quickPromptLayering() throws {
        ...
        let layered = try makeRepo([
            ...
            ".unifieddev/settings.toml": "[[quick_prompts]]\nname = \"Unified Dev\"\nprompt = \"Unified Dev words.\"\n",
            ".unifieddev/settings.local.toml": "[git]\nbranch_prefix = \"freek\"\n",
        ])
        #expect(SettingsLoader.load(repo: layered).quickPrompts.map(\.name) == ["Unified Dev"])

        let cleared = try makeRepo([
            ".unifieddev/settings.toml": shared,
            ".unifieddev/settings.local.toml": "quick_prompts = []\n",
        ])
    ...
    func homeFilesStateNoQuickPrompts() throws {
        ...
        SettingsLoader.apply(toml, from: "/home/.unifieddev/settings.toml", to: &settings)
        #expect(settings.quickPrompts.isEmpty)
    }
}
