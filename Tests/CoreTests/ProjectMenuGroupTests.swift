...
struct ProjectMenuGroupTests {
    ...
    func alphabeticalGroups() {
        let repos = [
            ...
            Repo(name: "unifieddev", path: "/unifieddev", sortOrder: 3),
            Repo(name: "unified-dev.akira-io.com", path: "/unified-dev.akira-io.com", sortOrder: 4)
        ]
        ...
        #expect(groups.map { $0.repos.map(\.id) } == [
            [repos[4].id, repos[3].id, repos[1].id],
    ...
    func emptyGroups(hidden: Bool) {
        let repo = Repo(name: "unifieddev", path: "/unifieddev", hidden: hidden)
