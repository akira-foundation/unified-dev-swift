...
struct WorkspaceContinuationTests {
    ...
    private func makeMergedWorkspace() async throws -> (
        ...
        let clonePath = TestScratch.unique("unifieddev-clone")
        try await Shell.check("git", ["clone", "-q", "--", origin.path, clonePath], cwd: origin.path)
        let repo = TempRepo(existing: clonePath)
        try await Shell.check("git", ["config", "user.email", "test@unifieddev.local"], cwd: repo.path)
        try await Shell.check("git", ["config", "user.name", "Unified Dev Test"], cwd: repo.path)
