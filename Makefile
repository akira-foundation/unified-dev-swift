...
run: ## Assemble a release UnifiedDev.app and launch it
...
master: ## Build HEAD and install it to ~/Applications/UnifiedDev.app
	./Tools/master.sh

dev: ## Build HEAD as Unified Dev (Dev), a second app that cannot reach the real data
	./Tools/dev-build.sh

dev-fast: ## Build current edits as Unified Dev (Dev) with an incremental debug build
...
dmg: ## Wrap the newest built UnifiedDev.app in the beach disk image
	./Tools/dmg/build.sh
